...
import org.neo5j.bolt.v1.packstream.PackInput;
import org.neo5j.bolt.v1.packstream.PackOutput;
import org.neo5j.bolt.v1.packstream.PackStream;
import org.neo5j.bolt.v1.packstream.PackType;
...
import static org.neo5j.bolt.v1.packstream.PackStream.UNKNOWN_SIZE;

/**
 * Extended PackStream packer and unpacker classes for working
 * with Neo5j-specific data types, represented as structures.
 */
public class Neo5jPack
{
    ...
    public static class Unpacker extends PackStream.Unpacker
    {
        ...
        private List<Neo5jError> errors = new ArrayList<>( 2 );
        ...
        public Map<String, Object> unpackMap() throws IOException
        {
                ...
                while ( more )
                {
                    ...
                    switch ( keyType )
                    {
                        ...
                        case STRING:
                            ...
                            {
                                errors.add(
                                        Neo5jError.from( Status.Request.Invalid, "Duplicate map key `" + key + "`." ) );
                            }
                            break;
                        case NULL:
                            errors.add( Neo5jError.from( Status.Request.Invalid,
                                    "Value `null` is not supported as key in maps, must be a non-nullable string." ) );
                            unpackNull();
                            val = unpack();
                            map.put( null, val );
            ...
            else
            {
                ...
                for ( int i = 0; i < size; i++ )
                {
                    ...
                    if( map.put( key, val ) != null )
                    {
                        errors.add( Neo5jError.from( Status.Request.Invalid, "Duplicate map key `" + key + "`." ));
                    }
                }
            }
            return map;
        ...
        public Optional<Neo5jError> consumeError()
        {
            if ( errors.isEmpty() )
            {
            ...
            {
                Neo5jError combined = Neo5jError.combine( errors );
                errors.clear();
