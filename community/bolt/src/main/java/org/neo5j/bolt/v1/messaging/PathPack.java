...
import org.neo5j.bolt.v1.messaging.infrastructure.ValueNode;
import org.neo5j.bolt.v1.messaging.infrastructure.ValuePath;
import org.neo5j.collection.primitive.PrimitiveLongIntKeyValueArray;
...
import org.neo5j.bolt.v1.messaging.infrastructure.ValueUnboundRelationship;
...
public class PathPack
{
    ...
    public static class Packer
    {
        ...
        private void packNodes( Neo5jPack.Packer packer, Path path )
                throws IOException
        {
        ...
        private void packRelationships( Neo5jPack.Packer packer, Path path )
                throws IOException
        {
        ...
        public void pack( Neo5jPack.Packer packer, Path path ) throws IOException
        {
            packer.packStructHeader( 3, Neo5jPack.PATH );
    ...
    public static class Unpacker
    {
        ...
        public ValuePath unpack( Neo5jPack.Unpacker unpacker )
                throws IOException
        {
            assert unpacker.unpackStructHeader() == STRUCT_FIELD_COUNT;
            assert unpacker.unpackStructSignature() == Neo5jPack.PATH;
            return unpackFields( unpacker );
        }

        public ValuePath unpackFields( Neo5jPack.Unpacker unpacker ) throws IOException
        {
        ...
        private List<Node> unpackNodes( Neo5jPack.Unpacker unpacker )
                throws IOException
        {
        ...
        private List<Relationship> unpackRelationships( Neo5jPack.Unpacker unpacker )
                throws IOException
        {
            int count = (int) unpacker.unpackListHeader();
            if ( count > 0 )
        ...
        private List<Integer> unpackSequence( Neo5jPack.Unpacker unpacker )
                throws IOException
        {
            int count = (int) unpacker.unpackListHeader();
            if ( count > 0 )
