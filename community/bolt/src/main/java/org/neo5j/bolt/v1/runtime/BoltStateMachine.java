...
import org.neo5j.graphdb.security.AuthProviderTimeoutException;
import org.neo5j.graphdb.security.AuthorizationExpiredException;
import org.neo5j.kernel.api.bolt.ManagedBoltStateMachine;
...
public class BoltStateMachine implements AutoCloseable, ManagedBoltStateMachine
{
    ...
    public void externalError( Neo5jError error, BoltResponseHandler handler ) throws BoltConnectionFatality
    {
        before( handler );
        try
        {
    ...
    public enum State
    {
        ...
        CONNECTED
                {
                                       ...
                                       Map<String, Object> authToken ) throws BoltConnectionFatality
                    {
                        ...
                        {
                            fail( machine, Neo5jError.fatalFrom( e.status(), e.getMessage() ) );
                            throw new BoltConnectionAuthFatality( e.getMessage() );
                        }
                        catch ( Throwable t )
                        {
                            fail( machine, Neo5jError.fatalFrom( Status.General.UnknownError, t.getMessage() ) );
        ...
        READY
                {
                                      ...
                                      Map<String, Object> params ) throws BoltConnectionFatality
                    {
                        ...
                        {
                            fail( machine, Neo5jError.fatalFrom( e ) );
                            throw new BoltConnectionAuthFatality( e.getMessage() );
                        }
                        catch ( Throwable e )
                        {
                            fail( machine, Neo5jError.from( e ) );
                            return FAILED;
                        }
                    }
                ...
                throws BoltConnectionFatality
        {
            ...
            fail( machine, Neo5jError.fatalFrom( Status.Request.Invalid, msg ) );
    ...
    private static void fail( BoltStateMachine machine, Neo5jError neo5jError )
    {
        machine.spi.reportError( neo5jError );
        machine.ctx.markFailed( neo5jError );
    }
    ...
    static class MutableConnectionState implements BoltResponseHandler
    {
        ...
        @Override
        public void markFailed( Neo5jError error )
        {
            if ( responseHandler != null )
            {
    ...
    interface SPI
    {
        void reportError( Neo5jError err );
