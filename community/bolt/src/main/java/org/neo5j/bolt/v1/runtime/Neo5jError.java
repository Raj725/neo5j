...
public class Neo5jError
{
    ...
    private Neo5jError( Status status, String message, Throwable cause, boolean fatal )
    {
        this.status = status;
    ...
    private Neo5jError( Status status, String message, boolean fatal )
    {
        this(status, message, null, fatal );
    }

    private Neo5jError( Status status, Throwable cause, boolean fatal )
    {
    ...
    private static Neo5jError fromThrowable( Throwable any, boolean isFatal )
    {
        for( Throwable cause = any; cause != null; cause = cause.getCause() )
        {
            if ( cause instanceof Status.HasStatus )
            {
                return new Neo5jError( ((Status.HasStatus) cause).status(), any.getMessage(), any, isFatal );
            }
            if ( cause instanceof DatabaseShutdownException )
            {
                return new Neo5jError( Status.General.DatabaseUnavailable, cause, isFatal );
            }
            if (cause instanceof OutOfMemoryError)
            {
                return new Neo5jError( Status.General.OutOfMemoryError, cause, isFatal );
            }
            if (cause instanceof StackOverflowError)
            {
                return new Neo5jError( Status.General.StackOverFlowError, cause, isFatal );
            }
        }
        ...
        return new Neo5jError( Status.General.UnknownError, any.getMessage(), any, isFatal );
    }

    public static Neo5jError from( Status status, String message )
    {
        return new Neo5jError( status, message, false );
    }

    public static Neo5jError from( Throwable any )
    {
        return fromThrowable( any, false );
    }

    public static Neo5jError combine( List<Neo5jError> errors )
    {
    ...
    public static Neo5jError fatalFrom( Throwable any )
    {
        return fromThrowable( any, true );
    }

    public static Neo5jError fatalFrom( Status status, String message )
    {
        return new Neo5jError( status, message, true );
    }
