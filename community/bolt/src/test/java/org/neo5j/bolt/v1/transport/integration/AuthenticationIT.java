 ...
 */
package org.neo5j.bolt.v1.transport.integration;
...
import org.neo5j.bolt.v1.messaging.message.AckFailureMessage;
import org.neo5j.bolt.v1.messaging.message.FailureMessage;
import org.neo5j.bolt.v1.messaging.message.InitMessage;
import org.neo5j.bolt.v1.messaging.message.PullAllMessage;
import org.neo5j.bolt.v1.messaging.message.ResetMessage;
import org.neo5j.bolt.v1.messaging.message.ResponseMessage;
import org.neo5j.bolt.v1.messaging.message.RunMessage;
import org.neo5j.bolt.v1.transport.socket.client.SecureSocketConnection;
import org.neo5j.bolt.v1.transport.socket.client.SecureWebSocketConnection;
import org.neo5j.bolt.v1.transport.socket.client.SocketConnection;
import org.neo5j.bolt.v1.transport.socket.client.TransportConnection;
import org.neo5j.bolt.v1.transport.socket.client.WebSocketConnection;
...
import static org.neo5j.bolt.v1.messaging.util.MessageMatchers.msgIgnored;
...
public class AuthenticationIT
{
    ...
    protected Neo5jWithSocket server = new Neo5jWithSocket( getClass(), getTestGraphDatabaseFactory(),
    ...
    private final String version = "Neo5j/" + Version.getNeo5jVersion();
        ...
        client.connect( address )
                ...
                .send( TransportTestUtil.chunk(
                        InitMessage.init( "TestClient/1.1",
                                map( "principal", "neo5j", "credentials", "neo5j", "scheme", "basic" ) ) ) );
    ...
    public void shouldFailIfWrongCredentials() throws Throwable
    {
        ...
        client.connect( address )
                ...
                .send( TransportTestUtil.chunk(
                        InitMessage.init( "TestClient/1.1",
                                map( "principal", "neo5j", "credentials", "wrong", "scheme", "basic" ) ) ) );
    ...
    public void shouldFailIfWrongCredentialsFollowingSuccessfulLogin() throws Throwable
    {
        ...
        client.connect( address )
                .send( TransportTestUtil.acceptedVersions( 1, 0, 0, 0 ) )
                .send( TransportTestUtil.chunk(
                        InitMessage.init( "TestClient/1.1", map( "principal", "neo5j",
                                "credentials", "neo5j", "new_credentials", "secret", "scheme", "basic" ) ) ) );
        // Then
        assertThat( client, eventuallyReceives( new byte[]{0, 0, 0, 1} ) );
        assertThat( client, eventuallyReceives( msgSuccess() ) );
        ...
        client.connect( address )
                ...
                .send( TransportTestUtil.chunk(
                        InitMessage.init( "TestClient/1.1",
                                map( "principal", "neo5j", "credentials", "secret", "scheme", "basic" ) ) ) );
    ...
    public void shouldFailIfMalformedAuthTokenWrongType() throws Throwable
    {
        ...
        client.connect( address )
                ...
                .send( TransportTestUtil.chunk(
                        InitMessage.init( "TestClient/1.1",
                                map( "principal", singletonList( "neo5j" ), "credentials", "neo5j", "scheme", "basic" ) ) ) );
    ...
    public void shouldFailIfMalformedAuthTokenMissingKey() throws Throwable
    {
        ...
        client.connect( address )
                ...
                .send( TransportTestUtil.chunk(
                        InitMessage.init( "TestClient/1.1",
                                map( "principal", "neo5j", "this-should-have-been-credentials", "neo5j", "scheme", "basic" ) ) ) );
    ...
    public void shouldFailIfMalformedAuthTokenMissingScheme() throws Throwable
    {
        ...
        client.connect( address )
                ...
                .send( TransportTestUtil.chunk(
                        InitMessage.init( "TestClient/1.1",
                                map( "principal", "neo5j", "credentials", "neo5j" ) ) ) );
    ...
    public void shouldFailDifferentlyIfTooManyFailedAuthAttempts() throws Exception
    {
        ...
        while ( System.currentTimeMillis() < timeout && !failureMatcher.gotSpecialMessage() )
        {
            ...
            client.connect( address )
                    ...
                    .send( TransportTestUtil.chunk(
                            InitMessage.init( "TestClient/1.1",
                                    map( "principal", "neo5j", "credentials", "WHAT_WAS_THE_PASSWORD_AGAIN",
                                            "scheme", "basic" ) ) ) );
    ...
    public void shouldFailWhenReusingTheSamePassword() throws Throwable
    {
        ...
        // When
        client.send( TransportTestUtil.chunk(
                RunMessage.run( "CALL dbms.security.changePassword", Collections.singletonMap( "password", "neo5j" ) ),
                PullAllMessage.pullAll() ) );
