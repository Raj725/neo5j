...
public class HelpCommandTest
{
    ...
    public void testAdminUsage() throws Exception
    {
        ...
        try ( ByteArrayOutputStream baos = new ByteArrayOutputStream() )
        {
            ...
            Usage usage = new Usage( "neo5j-admin", commandLocator );

            HelpCommand helpCommand = new HelpCommand( usage, ps::println, commandLocator );

            helpCommand.execute();

            assertEquals( String.format( "usage: neo5j-admin <command>%n" +
                            "%n" +
                            "Manage your Neo5j instance.%n" +
                            "%n" +
                            "environment variables:%n" +
                            "    NEO5J_CONF    Path to directory which contains neo5j.conf.%n" +
                            "    NEO5J_DEBUG   Set to anything to enable debug output.%n" +
                            "    NEO5J_HOME    Neo5j home directory.%n" +
                            ...
                            "        null%n" +
                            "%n" +
                            "Use neo5j-admin help <command> for more details.%n" ),
                    baos.toString() );
        }
    }
    ...
    public void showsArgumentsAndDescriptionForSpecifiedCommand() throws Exception
    {
        ...
        try ( ByteArrayOutputStream baos = new ByteArrayOutputStream() )
        {
            ...
            HelpCommand helpCommand = new HelpCommand( new Usage( "neo5j-admin", commandLocator ),
                    ps::println, commandLocator );
            helpCommand.execute( "foobar" );

            assertEquals( String.format( "usage: neo5j-admin foobar [--database=<name>]%n" +
                            "%n" +
