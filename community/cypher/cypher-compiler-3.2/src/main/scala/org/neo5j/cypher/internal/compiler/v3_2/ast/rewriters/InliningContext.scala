...
import org.neo5j.cypher.internal.compiler.v3_2.ast.rewriters.InliningContext._
import org.neo5j.cypher.internal.frontend.v3_2.ast._
import org.neo5j.cypher.internal.frontend.v3_2.ast.rewriters.copyVariables
import org.neo5j.cypher.internal.frontend.v3_2.{Rewriter, TypedRewriter, bottomUp}
...
case class InliningContext(projections: Map[Variable, Expression] = Map.empty,
  ...
  def patternRewriter: Rewriter = bottomUp(Rewriter.lift {
    ...
    case rel @ RelationshipPattern(Some(ident), _, _, _, _) if okToRewrite(ident) =>
      alias(ident) match {
