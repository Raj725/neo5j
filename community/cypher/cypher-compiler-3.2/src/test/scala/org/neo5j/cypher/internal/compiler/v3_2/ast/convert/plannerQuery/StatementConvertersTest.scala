 ...
 */
package org.neo5j.cypher.internal.compiler.v3_2.ast.convert.plannerQuery

import org.neo5j.cypher.internal.compiler.v3_2.planner.{LogicalPlanningTestSupport, _}
import org.neo5j.cypher.internal.compiler.v3_2.spi._
import org.neo5j.cypher.internal.frontend.v3_2.SemanticDirection.{BOTH, INCOMING, OUTGOING}
...
class StatementConvertersTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS collected WHERE (owner)--() RETURN owner") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(IdName(owner)),Set(),Selections(Set()),Vector(),Set(),Set(),List()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(collected -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collected)),Selections(Set()),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(owner -> Variable(owner), collected -> Variable(collected),   FRESHID54 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED62)),List(),None,None,BOTH),NodePattern(Some(Variable(  UNNAMED64)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collected), IdName(  FRESHID54)),Selections(Set(Predicate(Set(IdName(  FRESHID54)),Variable(  FRESHID54)))),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))"""
