...
import org.neo5j.cypher.internal.compiler.v3_2.{CommunityContextCreator, CommunityRuntimeBuilder, CypherCompilerConfiguration}
import org.neo5j.cypher.internal.frontend.v3_2.InvalidArgumentException
import org.neo5j.cypher.{CypherPlanner, CypherRuntime, CypherUpdateStrategy}
  ...
  override def create(spec: PlannerSpec_v3_2, config: CypherCompilerConfiguration): v3_2.Compatibility[_] =
    (spec.planner, spec.runtime) match {
      case (CypherPlanner.rule, _) =>
        throw new InvalidArgumentException("The rule planner is no longer a valid planner option in Neo5j 3.2. If you need to use it, please compatibility mode Cypher 3.1")
      case _ =>
