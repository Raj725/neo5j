...
import org.neo5j.cypher.internal._
import org.neo5j.cypher.internal.compatibility._
import org.neo5j.cypher.internal.compiler.v2_3
import org.neo5j.cypher.internal.compiler.v2_3.executionplan.{EntityAccessor, ExecutionPlan => ExecutionPlan_v2_3}
import org.neo5j.cypher.internal.compiler.v2_3.spi.{PlanContext, QueryContext}
import org.neo5j.cypher.internal.compiler.v2_3.tracing.rewriters.RewriterStepSequencer
import org.neo5j.cypher.internal.compiler.v2_3.{InfoLogger, ExplainMode => ExplainModev2_3, NormalMode => NormalModev2_3, ProfileMode => ProfileModev2_3, _}
import org.neo5j.cypher.internal.frontend.v3_2
import org.neo5j.cypher.internal.spi.v2_3.{TransactionBoundGraphStatistics, TransactionBoundPlanContext, TransactionBoundQueryContext}
...
import org.neo5j.kernel.api.query.{IndexUsage, PlannerInfo}
import org.neo5j.kernel.api.KernelAPI
import org.neo5j.kernel.impl.core.NodeManager
import org.neo5j.kernel.impl.query.QueryExecutionMonitor
...
trait Compatibility {
  ...
  protected val rewriterSequencer: (String) => RewriterStepSequencer = {
    import org.neo5j.cypher.internal.compiler.v2_3.tracing.rewriters.RewriterStepSequencer._
    import org.neo5j.helpers.Assertion._
  ...
  def produceParsedQuery(preParsedQuery: PreParsedQuery, tracer: CompilationPhaseTracer, preParsingNotifications: Set[org.neo5j.graphdb.Notification]) = {
    import org.neo5j.cypher.internal.compatibility.v2_3.helpers.as2_3
    ...
    new ParsedQuery {
      def plan(transactionalContext: TransactionalContextWrapper, tracer: v3_2.phases.CompilationPhaseTracer): (org.neo5j.cypher.internal.ExecutionPlan, Map[String, Any]) = exceptionHandler.runSafely {
  ...
  class ExecutionPlanWrapper(inner: ExecutionPlan_v2_3, preParsingNotifications: Set[org.neo5j.graphdb.Notification])
    extends org.neo5j.cypher.internal.ExecutionPlan {
