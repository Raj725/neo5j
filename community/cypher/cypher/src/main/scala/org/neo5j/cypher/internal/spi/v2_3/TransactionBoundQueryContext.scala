 ...
 */
package org.neo5j.cypher.internal.spi.v2_3

import java.net.URL
import java.util.function.Predicate

import org.neo5j.collection.primitive.PrimitiveLongIterator
import org.neo5j.collection.primitive.base.Empty.EMPTY_PRIMITIVE_LONG_COLLECTION
import org.neo5j.cypher.InternalException
import org.neo5j.cypher.internal.compiler.v2_3.MinMaxOrdering.{BY_NUMBER, BY_STRING, BY_VALUE}
import org.neo5j.cypher.internal.compiler.v2_3._
import org.neo5j.cypher.internal.compiler.v2_3.ast.convert.commands.DirectionConverter.toGraphDb
import org.neo5j.cypher.internal.compiler.v2_3.commands.expressions
import org.neo5j.cypher.internal.compiler.v2_3.commands.expressions.{KernelPredicate, OnlyDirectionExpander, TypeAndDirectionExpander}
import org.neo5j.cypher.internal.compiler.v2_3.helpers.JavaConversionSupport
import org.neo5j.cypher.internal.compiler.v2_3.helpers.JavaConversionSupport._
import org.neo5j.cypher.internal.compiler.v2_3.pipes.matching.PatternNode
import org.neo5j.cypher.internal.compiler.v2_3.spi._
import org.neo5j.cypher.internal.frontend.v2_3.{Bound, EntityNotFoundException, FailedIndexException, SemanticDirection}
import org.neo5j.cypher.internal.spi.BeansAPIRelationshipIterator
import org.neo5j.cypher.internal.spi.v3_2.TransactionalContextWrapper
import org.neo5j.cypher.javacompat.internal.GraphDatabaseCypherService
import org.neo5j.graphalgo.impl.path.ShortestPath
import org.neo5j.graphalgo.impl.path.ShortestPath.ShortestPathPredicate
import org.neo5j.graphdb.RelationshipType._
import org.neo5j.graphdb._
import org.neo5j.graphdb.security.URLAccessValidationError
import org.neo5j.graphdb.traversal.{Evaluators, TraversalDescription, Uniqueness}
import org.neo5j.kernel.GraphDatabaseQueryService
import org.neo5j.kernel.api.constraints.{NodePropertyExistenceConstraint, RelationshipPropertyExistenceConstraint, UniquenessConstraint}
import org.neo5j.kernel.api.exceptions.schema.{AlreadyConstrainedException, AlreadyIndexedException}
...
import org.neo5j.kernel.api.schema_new.constaints.{ConstraintBoundary, ConstraintDescriptorFactory}
import org.neo5j.kernel.api.schema_new.index.NewIndexDescriptorFactory
import org.neo5j.kernel.api.{exceptions, _}
...
final class TransactionBoundQueryContext(tc: TransactionalContextWrapper)
  ...
  def indexSeek(index: IndexDescriptor, value: Any) =
    JavaConversionSupport.mapToScalaENFXSafe(tc.statement.readOperations().indexQuery(index, IndexQuery.exact(index.property, value)))(nodeOps.getById)
  ...
  private def indexSeekByPrefixRange(index: IndexDescriptor, prefix: String): scala.Iterator[Node] = {
    val indexedNodes = tc.statement.readOperations().indexQuery(index, IndexQuery.stringPrefix(index.property, prefix))
  ...
  override def lockingExactUniqueIndexSearch(index: IndexDescriptor, value: Any): Option[Node] = {
    val nodeId: Long = tc.statement.readOperations().nodeGetFromUniqueIndexSeek(index, IndexQuery.exact(index.property, value))
  ...
  class NodeOperations extends BaseOperations[Node] {
    ...
    def propertyKeyIds(id: Long): Iterator[Int] = try {
      ...
      new Iterator[Int] {
        ...
        override def next(): Int = try {
          ...
          case _: org.neo5j.kernel.api.exceptions.EntityNotFoundException => null.asInstanceOf[Int]
        }
      }
    } catch {
    ...
    def hasProperty(id: Long, propertyKey: Int) = try {
      ...
      case _: org.neo5j.kernel.api.exceptions.EntityNotFoundException => false
    }

    def removeProperty(id: Long, propertyKeyId: Int) = try {
      tc.statement.dataWriteOperations().nodeRemoveProperty(id, propertyKeyId)
    } catch {
      case _: org.neo5j.kernel.api.exceptions.EntityNotFoundException => //ignore
    }
