...
import org.neo5j.graphdb.{NotFoundException, TransactionFailureException}
      ...
      val threads: List[MyThread] = ids.map { id =>
        ...
        new MyThread(() => {
          ...
          case ex: TransactionFailureException =>
            ...
            cause.isInstanceOf[org.neo5j.kernel.api.exceptions.TransactionFailureException] &&
              cause.getMessage == "Transaction rolled back even if marked as successful"
          case ex: CypherExecutionException =>
            ex.getCause.isInstanceOf[org.neo5j.kernel.api.exceptions.EntityNotFoundException] &&
