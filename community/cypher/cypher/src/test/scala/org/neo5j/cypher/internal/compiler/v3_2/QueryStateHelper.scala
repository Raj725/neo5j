...
import org.neo5j.cypher.internal.compiler.v3_2.pipes.{ExternalCSVResource, NullPipeDecorator, PipeDecorator, QueryState}
import org.neo5j.cypher.internal.compiler.v3_2.spi.{QueryContext, UpdateCountingQueryContext}
...
object QueryStateHelper {
  ...
  def queryStateFrom(db: GraphDatabaseQueryService, tx: InternalTransaction, params: Map[String, Any] = Map.empty): QueryState = {
    ...
    val contextFactory = Neo5jTransactionalContextFactory.create(db, locker)
