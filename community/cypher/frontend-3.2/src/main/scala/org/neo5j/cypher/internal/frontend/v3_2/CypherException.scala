 ...
 */
package org.neo5j.cypher.internal.frontend.v3_2

import org.neo5j.cypher.internal.frontend.v3_2.ExhaustiveShortestPathForbiddenException.ERROR_MSG
import org.neo5j.cypher.internal.frontend.v3_2.spi.MapToPublicExceptions
...
object ExhaustiveShortestPathForbiddenException {
  val ERROR_MSG: String =
    s"""Shortest path fallback has been explicitly disabled. That means that no full path enumeration is performed in
       ...
       |label 'Person'. In case this is an unexpected error please either disable the runtime error in the Neo5j
       |configuration or please improve your query by consulting the Neo5j manual.  In order to avoid planning the
...
object ShortestPathCommonEndNodesForbiddenException {
  val ERROR_MSG: String =
    s"""The shortest path algorithm does not work when the start and end nodes are the same. This can happen if you
       ...
       |possibility of missing results for those rows, disable this in the Neo5j configuration by setting
