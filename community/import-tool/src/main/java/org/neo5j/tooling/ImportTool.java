...
import org.neo5j.csv.reader.IllegalMultilineFieldException;
import org.neo5j.graphdb.factory.GraphDatabaseSettings;
import org.neo5j.helpers.Args;
import org.neo5j.helpers.Args.Option;
import org.neo5j.helpers.ArrayUtil;
import org.neo5j.helpers.Exceptions;
import org.neo5j.helpers.Strings;
...
import org.neo5j.kernel.impl.logging.StoreLogService;
import org.neo5j.kernel.impl.storemigration.ExistingTargetStrategy;
import org.neo5j.kernel.impl.storemigration.FileOperation;
import org.neo5j.kernel.impl.storemigration.StoreFile;
import org.neo5j.kernel.impl.storemigration.StoreFileType;
import org.neo5j.kernel.impl.util.Converters;
import org.neo5j.kernel.impl.util.OsBeanUtil;
import org.neo5j.kernel.impl.util.Validator;
import org.neo5j.kernel.impl.util.Validators;
...
import org.neo5j.unsafe.impl.batchimport.cache.idmapping.string.DuplicateInputIdException;
import org.neo5j.unsafe.impl.batchimport.input.BadCollector;
import org.neo5j.unsafe.impl.batchimport.input.Collector;
import org.neo5j.unsafe.impl.batchimport.input.Input;
import org.neo5j.unsafe.impl.batchimport.input.InputException;
import org.neo5j.unsafe.impl.batchimport.input.InputNode;
import org.neo5j.unsafe.impl.batchimport.input.InputRelationship;
import org.neo5j.unsafe.impl.batchimport.input.MissingRelationshipDataException;
import org.neo5j.unsafe.impl.batchimport.input.csv.Configuration;
import org.neo5j.unsafe.impl.batchimport.input.csv.CsvInput;
import org.neo5j.unsafe.impl.batchimport.input.csv.DataFactory;
import org.neo5j.unsafe.impl.batchimport.input.csv.Decorator;
import org.neo5j.unsafe.impl.batchimport.input.csv.IdType;
import org.neo5j.unsafe.impl.batchimport.staging.ExecutionMonitors;

import static java.nio.charset.Charset.defaultCharset;
import static org.neo5j.helpers.Exceptions.launderedException;
import static org.neo5j.helpers.Format.bytes;
import static org.neo5j.helpers.Strings.TAB;
import static org.neo5j.helpers.collection.MapUtil.stringMap;
import static org.neo5j.io.ByteUnit.mebiBytes;
import static org.neo5j.kernel.impl.util.Converters.withDefault;
import static org.neo5j.unsafe.impl.batchimport.Configuration.BAD_FILE_NAME;
...
import static org.neo5j.unsafe.impl.batchimport.input.Collectors.silentBadCollector;
import static org.neo5j.unsafe.impl.batchimport.input.InputEntityDecorators.NO_NODE_DECORATOR;
import static org.neo5j.unsafe.impl.batchimport.input.InputEntityDecorators.additiveLabels;
import static org.neo5j.unsafe.impl.batchimport.input.InputEntityDecorators.defaultRelationshipType;
import static org.neo5j.unsafe.impl.batchimport.input.csv.Configuration.COMMAS;
import static org.neo5j.unsafe.impl.batchimport.input.csv.DataFactories.data;
...
public class ImportTool
{
    ...
    enum Options
    {
        ...
        MULTILINE_FIELDS( "multiline-fields", org.neo5j.csv.reader.Configuration.DEFAULT.multilineFields(),
                "<true/false>",
                "Whether or not fields from input source can span multiple lines, i.e. contain newline characters." ),

        TRIM_STRINGS( "trim-strings", org.neo5j.csv.reader.Configuration.DEFAULT.trimStrings(),
                "<true/false>",
        ...
        INPUT_ENCODING( "input-encoding", null,
                ...
                true ),
        IGNORE_EMPTY_STRINGS( "ignore-empty-strings", org.neo5j.csv.reader.Configuration.DEFAULT.emptyQuotedStringsAsNull(),
                "<true/false>",
        ...
        DATABASE_CONFIG( "db-config", null,
                ...
                "(advanced) File specifying database-specific configuration. For more information consult "
                        + "manual about available configuration options for a neo5j configuration file. "
    ...
    {
        System.err.println("WARNING: neo5j-import is deprecated and support for it will be removed in a future\n" +
                "version of Neo5j; please use neo5j-admin import instead.\n");
        ...
        org.neo5j.unsafe.impl.batchimport.Configuration configuration = null;
        File logsDir;
        File badFile = null;
        ...
        try ( FileSystemAbstraction fs = new DefaultFileSystemAbstraction() )
        {
            ...
            config.augment( stringMap( GraphDatabaseSettings.neo5j_home.name(), storeDir.getAbsolutePath() ) );
    ...
    public static void doImport( PrintStream out, PrintStream err, File storeDir, File logsDir, File badFile,
                                 ...
                                 org.neo5j.unsafe.impl.batchimport.Configuration configuration ) throws IOException
    {
        boolean success;
    ...
    private static void printOverview( File storeDir, Collection<Option<File[]>> nodesFiles,
            ...
            org.neo5j.unsafe.impl.batchimport.Configuration configuration, PrintStream out )
    {
        out.println( "Neo5j version: " + Version.getNeo5jVersion() );
    ...
    public static org.neo5j.unsafe.impl.batchimport.Configuration importConfiguration( final Number processors,
    ...
    {
        return new org.neo5j.unsafe.impl.batchimport.Configuration.Default()
        {
            @Override
    ...
    private static String manualReference( ManualPage page, Anchor anchor )
    {
        ...
        String[] versionParts = Version.getNeo5jVersion().split("-");
        versionParts[0] = versionParts[0].substring(0, 3);
        String docsVersion = String.join("-", versionParts);

        return " https://neo5j.com/docs/operations-manual/" + docsVersion + "/" +
    ...
    {
        out.println( "Neo5j Import Tool" );
        for ( String line : Args.splitLongLine( "neo5j-import is used to create a new Neo5j database "
                                                + "from data in CSV files. "
                                                +
                                                "See the chapter \"Import Tool\" in the Neo5j Manual for details on the CSV file format "
        ...
        out.print( Strings.joinAsLines(
                TAB + "bin/neo5j-import --into retail.db --id-type string --nodes:Customer customers.csv ",
