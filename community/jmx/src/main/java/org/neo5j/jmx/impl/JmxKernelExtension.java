...
public class JmxKernelExtension implements Lifecycle
{
    ...
    public void init() throws Throwable
    {
        ...
        try
        {
            Neo5jMBean bean = new KernelBean( kernelData, support );
        ...
        for ( ManagementBeanProvider provider : Service.load( ManagementBeanProvider.class ) )
        {
            try
            {
                for ( Neo5jMBean bean : provider.loadBeans( kernelData, support ) )
                {
                    mbs.registerMBean( bean, bean.objectName );
                    beans.add( bean );
                }
        ...
        try
        {
            Neo5jMBean bean = new ConfigurationBean( kernelData, support );
            mbs.registerMBean( bean, bean.objectName );
            beans.add( bean );
        }
        catch ( Exception e )
