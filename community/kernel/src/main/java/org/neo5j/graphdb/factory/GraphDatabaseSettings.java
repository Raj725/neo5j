...
import org.neo5j.configuration.ReplacedBy;
import org.neo5j.csv.reader.Configuration;
...
import org.neo5j.kernel.configuration.BoltConnectorValidator;
import org.neo5j.kernel.configuration.ConfigurationMigrator;
import org.neo5j.kernel.configuration.GraphDatabaseConfigurationMigrator;
import org.neo5j.kernel.configuration.Group;
import org.neo5j.kernel.configuration.GroupSettingSupport;
import org.neo5j.kernel.configuration.HttpConnectorValidator;
import org.neo5j.configuration.Internal;
import org.neo5j.kernel.configuration.Migrator;
import org.neo5j.kernel.configuration.Settings;
import org.neo5j.kernel.configuration.Title;
import org.neo5j.kernel.impl.cache.MonitorGc;
import org.neo5j.logging.Level;

import static org.neo5j.helpers.collection.Iterables.enumNames;
import static org.neo5j.kernel.configuration.Settings.ANY;
import static org.neo5j.kernel.configuration.Settings.BOOLEAN;
import static org.neo5j.kernel.configuration.Settings.BYTES;
import static org.neo5j.kernel.configuration.Settings.DEFAULT;
import static org.neo5j.kernel.configuration.Settings.DOUBLE;
...
import static org.neo5j.kernel.configuration.Settings.INTEGER;
import static org.neo5j.kernel.configuration.Settings.LONG;
import static org.neo5j.kernel.configuration.Settings.NO_DEFAULT;
import static org.neo5j.kernel.configuration.Settings.PATH;
import static org.neo5j.kernel.configuration.Settings.STRING;
import static org.neo5j.kernel.configuration.Settings.STRING_LIST;
import static org.neo5j.kernel.configuration.Settings.TRUE;
import static org.neo5j.kernel.configuration.Settings.advertisedAddress;
import static org.neo5j.kernel.configuration.Settings.derivedSetting;
import static org.neo5j.kernel.configuration.Settings.illegalValueMessage;
...
import static org.neo5j.kernel.configuration.Settings.matches;
import static org.neo5j.kernel.configuration.Settings.max;
import static org.neo5j.kernel.configuration.Settings.min;
import static org.neo5j.kernel.configuration.Settings.options;
...
public class GraphDatabaseSettings implements LoadableConfig
{
    ...
    public static final Setting<File> neo5j_home =
            setting( "unsupported.dbms.directories.neo5j_home", PATH, NO_DEFAULT );

    @Title("Read only database")
    @Description("Only allow read operations from this Neo5j instance. " +
    ...
    @Description("Print out the effective Neo5j configuration after startup.")
    @Internal
    ...
    @Description( "A strict configuration validation will prevent the database from starting up if unknown " +
            "configuration options are specified in the neo5j settings namespace (such as dbms., ha., cypher., etc). " +
    ...
    @Description( "This setting is associated with performance optimization. Set this to `true` in situations where " +
                  ...
                  "which means that Neo5j will never consider using the exhaustive search for shortestPath queries. " +
    ...
    @Description( "Determines if Cypher will allow using file URLs when loading data using `LOAD CSV`. Setting this "
                  + "value to `false` will cause Neo5j to fail `LOAD CSV` clauses that load data from the file system." )
    ...
    @Description( "Selects whether to conform to the standard https://tools.ietf.org/html/rfc4180 for interpreting " +
                  ...
                  "use the legacy convention originally supported in Neo5j 3.0 and 3.1, allowing a backslash to " +
    ...
    public static final Setting<List<String>> store_internal_debug_contexts = setting( "unsupported.dbms.logs.debug.debug_loggers",
            list( ",", STRING ), "org.neo5j.diagnostics,org.neo5j.cluster.protocol,org.neo5j.kernel.ha" );
    ...
    @Description( "Limit the number of IOs the background checkpoint process will consume per second. " +
                  "This setting is advisory, is ignored in Neo5j Community Edition, and is followed to " +
    ...
    @Description("Size of buffer used by index sampling. " +
                 "This configuration setting is no longer applicable as from Neo5j 3.0.3. " +
    ...
    // Store settings
    @Description("Make Neo5j keep the logical transaction logs for being able to backup the database. " +
    ...
    @Description( "The amount of memory to use for mapping the store files, in bytes (or kilobytes with the 'k' " +
                  "suffix, megabytes with 'm' and gigabytes with 'g'). If Neo5j is running on a dedicated server, " +
    ...
    @Description( "Log executed queries that take longer than the configured threshold, dbms.logs.query.threshold. " +
            ...
            "This feature is available in the Neo5j Enterprise Edition." )
    ...
    @Description("Enable auth requirement to access Neo5j.")
    ...
    /**
     * DEPRECATED: Use {@link org.neo5j.kernel.configuration.BoltConnector} instead. This will be removed in 4.0.
     */
    @Deprecated
    ...
    /**
     * DEPRECATED: Use {@link org.neo5j.kernel.configuration.Connector} instead. This will be removed in 4.0.
     */
