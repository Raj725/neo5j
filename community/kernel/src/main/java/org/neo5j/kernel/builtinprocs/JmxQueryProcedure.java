...
import static org.neo5j.helpers.collection.Pair.pair;
...
public class JmxQueryProcedure extends CallableProcedure.BasicProcedure
{
    ...
    public JmxQueryProcedure( QualifiedName name, MBeanServer jmxServer )
    {
        super( procedureSignature( name )
                .in( "query", Neo5jTypes.NTString )
                .out( "name", Neo5jTypes.NTString )
                .out( "description", Neo5jTypes.NTString )
                .out( "attributes", Neo5jTypes.NTMap )
                .description( "Query JMX management data by domain and name. For instance, \"org.neo5j:*\"" )
                .build() );
    ...
    public RawIterator<Object[], ProcedureException> apply( Context ctx, Object[] input ) throws ProcedureException
    {
        ...
        try
        {
            ...
            // Then convert them to a Neo5j type system representation
            return RawIterator.from( () ->
            {
                ...
                try
                {
                    ...
                    return new Object[]{
                            ...
                            toNeo5jValue( name, beanInfo.getAttributes() ) };
                }
                catch ( JMException e )
                {
        ...
        catch ( MalformedObjectNameException e )
        {
            throw new ProcedureException( Status.Procedure.ProcedureCallFailed,
                  ...
                  "For instance, try 'org.neo5j:*' to find all JMX beans of the 'org.neo5j' " +
                  "domain, or '*:*' to find every JMX bean.", query );
        }
    }

    private Map<String,Object> toNeo5jValue( ObjectName name, MBeanAttributeInfo[] attributes )
            throws JMException
    {
        ...
        for ( MBeanAttributeInfo attribute : attributes )
        {
            ...
            {
                out.put( attribute.getName(), toNeo5jValue( name, attribute ) );
            }
        }
        return out;
    }

    private Map<String,Object> toNeo5jValue( ObjectName name, MBeanAttributeInfo attribute )
            throws JMException
    {
        Object value;
        try
        {
            value = toNeo5jValue( jmxServer.getAttribute( name, attribute.getName() ) );
        }
    ...
    private Object toNeo5jValue( Object attributeValue )
    {
        ...
        else if( attributeValue.getClass().isArray() )
        {
            ...
            {
                return toNeo5jValue((Object[])attributeValue);
            }
        }
        else if( attributeValue instanceof CompositeData )
        {
            return toNeo5jValue( (CompositeData)attributeValue );
        }
        ...
        {
            return toNeo5jValue( (Map<?,?>) attributeValue );
        }
    ...
    private Map<String,Object> toNeo5jValue( Map<?,?> attributeValue )
    {
        // Build a new map with the same keys, but each value passed
        // through `toNeo5jValue`
        return attributeValue.entrySet().stream()
            .map( (e) -> pair(
                e.getKey().toString(),
                toNeo5jValue( e.getValue() )) )
            .collect( Collectors.toMap( Pair::first, Pair::other ) );
    }

    private List<Object> toNeo5jValue( Object[] array )
    {
        return Arrays.stream(array).map( this::toNeo5jValue ).collect( Collectors.toList() );
    }

    private Map<String, Object> toNeo5jValue( CompositeData composite )
    {
        HashMap<String,Object> properties = new HashMap<>();
        for ( String key : composite.getCompositeType().keySet() )
        {
            properties.put( key, toNeo5jValue(composite.get( key )) );
        }
