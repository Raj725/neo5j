...
import org.neo5j.kernel.impl.proc.TypeMappers.NeoValueConverter;
import org.neo5j.procedure.Name;

/**
 * Given a java method, figures out a valid {@link org.neo5j.kernel.api.proc.ProcedureSignature} field signature.
 * Basically, it takes the java signature and spits out the same signature described as Neo5j types.
 */
public class MethodSignatureCompiler
{
    ...
    public List<Neo5jTypes.AnyType> inputTypesFor( Method method ) throws ProcedureException
    {
        Type[] types = method.getGenericParameterTypes();
        List<Neo5jTypes.AnyType> neoTypes = new ArrayList<>(types.length);
        for ( Type type : types )
        {
    ...
    public List<FieldSignature> signatureFor( Method method ) throws ProcedureException
    {
        ...
        for ( int i = 0; i < params.length; i++ )
        {
            ...
            {
                NeoValueConverter valueConverter = typeMappers.converterFor( type );
                Optional<Neo5jValue> defaultValue = valueConverter.defaultValue( parameter );
            ...
            catch ( ProcedureException e )
            {
                throw new ProcedureException( e.status(),
                        ...
                        "type `%s` cannot be converted to a Neo5j type: %s",
