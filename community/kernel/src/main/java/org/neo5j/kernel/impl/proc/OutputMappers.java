 ...
 * an instance of the class to Neo5j-typed Object[].
 */
public class OutputMappers
{
    ...
    /**
     * Extracts field value from an instance and converts it to a Neo5j typed value.
     */
    ...
    public OutputMapper mapper( Class<?> userClass ) throws ProcedureException
    {
        ...
        for ( int i = 0; i < fields.size(); i++ )
        {
            ...
            {
                throw new ProcedureException( e.status(), e, "Field `%s` in record `%s` cannot be converted to a Neo5j type: %s", field.getName(),
