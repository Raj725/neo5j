...
import static org.neo5j.kernel.api.proc.Neo5jTypes.NTAny;
import static org.neo5j.kernel.api.proc.Neo5jTypes.NTBoolean;
import static org.neo5j.kernel.api.proc.Neo5jTypes.NTFloat;
...
import static org.neo5j.kernel.api.proc.Neo5jTypes.NTNumber;
import static org.neo5j.kernel.api.proc.Neo5jTypes.NTString;
import static org.neo5j.kernel.impl.proc.Neo5jValue.ntBoolean;
import static org.neo5j.kernel.impl.proc.Neo5jValue.ntFloat;
import static org.neo5j.kernel.impl.proc.Neo5jValue.ntInteger;

public class TypeMappers
{
    /**
     * Converts a java object to the specified {@link #type() neo5j type}. In practice, this is
     * often the same java object - but this gives a guarantee that only java objects Neo5j can
    ...
    interface NeoValueConverter
    {
        ...
        Optional<Neo5jValue> defaultValue(Name parameter) throws ProcedureException;
    }
    ...
    private final NeoValueConverter TO_STRING = new SimpleConverter( NTString, String.class, Neo5jValue::ntString );
    ...
    private ProcedureException javaToNeoMappingError( Type cls )
    {
        ...
        return new ProcedureException( Status.Statement.TypeError,
                "Don't know how to map `%s` to the Neo5j Type System.%n" +
    ...
    public static class SimpleConverter implements NeoValueConverter
    {
        ...
        private final Function<String,Neo5jValue> defaultConverter;
        ...
        public SimpleConverter( AnyType type, Class<?> javaClass, Function<String,Neo5jValue> defaultConverter )
        {
            this.type = type;
        ...
        public Optional<Neo5jValue> defaultValue( Name parameter ) throws ProcedureException
        {
        ...
        private static Function<String,Neo5jValue> nullParser( Class<?> javaType, Neo5jTypes.AnyType neoType )
        {
            return s ->
            {
                if ( s.equalsIgnoreCase( "null" ) )
                {
                    return new Neo5jValue( null, neoType );
                }
                else
                {
