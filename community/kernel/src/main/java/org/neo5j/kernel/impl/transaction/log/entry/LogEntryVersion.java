 ...
 */
package org.neo5j.kernel.impl.transaction.log.entry;

import org.neo5j.kernel.impl.transaction.command.PhysicalLogCommandReaderV2_2_4;
import org.neo5j.storageengine.api.StorageCommand;
import org.neo5j.storageengine.api.WritableChannel;
 ...
 *
 * Here follows an explanation how log entry versioning in Neo5j works:
 *
 * In Neo5j transactions are written to a log. Each transaction consists of one or more log entries. Log entries
 * can be of one or more types, such as denoting start of a transaction, commands and committing the transaction.
 * Neo5j supports writing the latest/current log entry and reading log entries for all currently supported versions
 * of Neo5j. The way versioning is done has changed over the years.
 ...
 *   Starting with Neo5j version 2.1 a one-byte log entry version was introduced with every single log entry.
 ...
 * was the next byte. This to support rolling upgrades where two Neo5j versions in a cluster could be active
 ...
 * </ol>
 * Everything apart from that should just work and Neo5j should automatically support the new version as well.
 *
...
{
    // as of 2013-02-09: neo5j 2.0 Labels & Indexing
    V2_0( 0, LogEntryParsersV2_0.class ),
    // as of 2014-02-06: neo5j 2.1 Dense nodes, split by type/direction into groups
    V2_1( -1, LogEntryParsersV2_1.class ),
    // as of 2014-05-23: neo5j 2.2 Removal of JTA / unified data source
    V2_2( -2, LogEntryParsersV2_2.class ),
    // as of 2015-07-23: neo5j 2.2.4 legacy index command header has bigger id space
    ...
    // as of 2016-05-27: neo5j 2.2.10 legacy index IndexDefineCommand maps write size as short instead of byte
    ...
    // as of 2016-05-30: neo5j 2.3.5 legacy index IndexDefineCommand maps write size as short instead of byte
    ...
    // as of 2016-05-30: neo5j 3.0.2 legacy index IndexDefineCommand maps write size as short instead of byte
    // See comment for V2.2.10 for version number explanation
    // log entry layout hasn't changed since 2_3 so just use that one
