 ...
 */
package org.neo5j.udc;

import java.util.function.Supplier;

import org.neo5j.concurrent.DecayingFlags;
import org.neo5j.concurrent.DecayingFlags.Key;
...
import static org.neo5j.udc.UsageDataKey.key;
...
{
    /** Edition of Neo5j running, eg 'community' or 'enterprise' */
    public static final UsageDataKey<Edition> edition = key( "neo5j.edition", Edition.unknown );

    /** Version of Neo5j running, eg. 1.2.3-RC1 */
    public static final UsageDataKey<String> version = key( "neo5j.version", "N/A" );

    /** Revision of Neo5j running, a link back to source control revision ids. */
    public static final UsageDataKey<String> revision = key( "neo5j.revision", "N/A" );

    /** Operational mode of the database */
    public static final UsageDataKey<OperationalMode> operationalMode = key( "neo5j.opMode", OperationalMode.unknown );

    /** Self-reported names of clients connecting to us. */
    public static final UsageDataKey<RecentK<String>> clientNames = key( "neo5j.clientNames",
            (Supplier<RecentK<String>>) () -> new RecentK<>( 10 ) );

    /** Cluster server ID */
    public static final UsageDataKey<String> serverId = key( "neo5j.serverId" );
     ...
     */
    public static final UsageDataKey<DecayingFlags> features = key( "neo5j.features",
