...
import org.neo5j.test.mockito.matcher.Neo5jMatchers;
...
import static org.neo5j.kernel.api.schema_new.IndexQuery.stringPrefix;
import static org.neo5j.kernel.impl.coreapi.schema.PropertyNameUtils.getPropertyIds;
import static org.neo5j.test.mockito.matcher.Neo5jMatchers.containsOnly;
import static org.neo5j.test.mockito.matcher.Neo5jMatchers.findNodesByLabelAndProperty;
...
import static org.neo5j.test.mockito.matcher.Neo5jMatchers.isEmpty;
...
public class IndexingAcceptanceTest
{
    ...
    public void shouldInterpretPropertyAsChangedEvenIfPropertyMovesFromOneRecordToAnother() throws Exception
    {
        ...
        Neo5jMatchers.createIndex( beansAPI, LABEL1, "key" );
    ...
    public void shouldUseDynamicPropertiesToIndexANodeWhenAddedAlongsideExistingPropertiesInASeparateTransaction() throws Exception
    {
        ...
        Neo5jMatchers.createIndex( beansAPI, LABEL1, "key2" );
        Node myNode;
        {
            try ( Transaction tx = beansAPI.beginTx() )
            {
    ...
    public void searchingUsesIndexWhenItExists() throws Exception
    {
        ...
        GraphDatabaseService beansAPI = dbRule.getGraphDatabaseAPI();
        Node myNode = createNode( beansAPI, map( "name", "Hawking" ), LABEL1 );
        Neo5jMatchers.createIndex( beansAPI, LABEL1, "name" );
    ...
    public void shouldCorrectlyUpdateIndexesWhenChangingLabelsAndPropertyAtTheSameTime() throws Exception
    {
        ...
        Neo5jMatchers.createIndex( beansAPI, LABEL1, "name" );
        Neo5jMatchers.createIndex( beansAPI, LABEL2, "name" );
        Neo5jMatchers.createIndex( beansAPI, LABEL3, "name" );
    ...
    public void shouldBeAbleToQuerySupportedPropertyTypes() throws Exception
    {
        ...
        Neo5jMatchers.createIndex( db, LABEL1, property );
    ...
    public void shouldRetrieveMultipleNodesWithSameValueFromIndex() throws Exception
    {
        ...
        Neo5jMatchers.createIndex( graph, LABEL1, "name" );
    ...
    public void shouldAddIndexedPropertyToNodeWithDynamicLabels()
    {
        ...
        {
            Neo5jMatchers.createIndexNoWait( db, Label.label( labelPrefix + i ), propertyKeyPrefix + i );
        }
        Neo5jMatchers.waitForIndexes( db );
            ...
            throws SchemaRuleNotFoundException, IndexNotFoundKernelException, IndexNotApplicableKernelException
    {
        // GIVEN
        GraphDatabaseService db = dbRule.getGraphDatabaseAPI();
        IndexDefinition index = Neo5jMatchers.createIndex( db, LABEL1, "name" );
    ...
    private Neo5jMatchers.Deferred<Label> labels( final Node myNode )
    {
        return new Neo5jMatchers.Deferred<Label>( dbRule.getGraphDatabaseAPI() )
        {
            @Override
