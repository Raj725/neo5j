...
public class MethodSignatureCompilerTest
{
    ...
    public void shouldMapSimpleRecordWithString() throws Throwable
    {
        ...
        // THen
        assertThat(signature, contains( new FieldSignature("name", Neo5jTypes.NTString)));
    }
    ...
    public void shouldMapSimpleFunctionWithString() throws Throwable
    {
        // When
        Method echo = ClassWithProcedureWithSimpleArgs.class.getMethod( "echo", String.class );
        List<Neo5jTypes.AnyType> signature = new MethodSignatureCompiler( new TypeMappers() ).inputTypesFor( echo );

        // THen
        assertThat(signature, contains( Neo5jTypes.NTString));
    }
    ...
    public void shouldGiveHelpfulErrorOnUnmappable() throws Throwable
    {
        ...
        exception.expectMessage( String.format("Argument `name` at position 0 in `echoWithInvalidType` with%n" +
                                 "type `UnmappableRecord` cannot be converted to a Neo5j type: Don't know how to map " +
                                 "`org.neo5j.kernel.impl.proc.MethodSignatureCompilerTest$UnmappableRecord` to " +
                                 "the Neo5j Type System.%n" +
