...
import org.neo5j.kernel.impl.proc.OutputMappers.OutputMapper;
...
public class OutputMappersTest
{
    ...
    public void shouldMapSimpleRecordWithString() throws Throwable
    {
        ...
        assertThat(
                mapper.signature(),
                contains( new FieldSignature( "name", Neo5jTypes.NTString ) )
        );
        assertThat(
    ...
    public void shouldSkipStaticFields() throws Throwable
    {
        ...
        assertThat(
                mapper.signature(),
                contains( new FieldSignature( "includeMe", Neo5jTypes.NTString ) )
        );
        assertThat(
    ...
    public void shouldGiveHelpfulErrorOnUnmappable() throws Throwable
    {
        ...
        exception.expectMessage(
                "Field `wat` in record `UnmappableRecord` cannot be converted to a Neo5j type: Don't know how to map `org.neo5j.kernel.impl.proc" +
    ...
    public void shouldGiveHelpfulErrorOnMapWithNonStringKeyMap() throws Throwable
    {
        // Expect
        exception.expect( ProcedureException.class );
        exception.expectMessage( "Field `wat` in record `RecordWithNonStringKeyMap` cannot be converted to a Neo5j type: Maps are required to have `String` " +
                                 "keys - but this map has `org.neo5j.kernel.impl.proc.OutputMappersTest$RecordWithNonStringKeyMap` keys." );
