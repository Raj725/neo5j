...
import org.neo5j.kernel.api.proc.CallableUserFunction;
...
public class ProcedureJarLoaderTest
{
    ...
    public void shouldLoadProcedureFromJar() throws Throwable
    {
        ...
        assertThat( signatures, contains(
                procedureSignature( "org","neo5j", "kernel", "impl", "proc", "myProcedure" ).out( "someNumber", NTInteger ).build() ));
    ...
    public void shouldLoadProcedureWithArgumentFromJar() throws Throwable
    {
        ...
        List<ProcedureSignature> signatures = procedures.stream().map( CallableProcedure::signature ).collect( toList() );
        assertThat( signatures, contains(
                procedureSignature( "org","neo5j", "kernel", "impl", "proc", "myProcedure" )
                        .in( "value", NTInteger )
    ...
    public void shouldLoadProcedureFromJarWithMultipleProcedureClasses() throws Throwable
    {
        ...
        assertThat( signatures, containsInAnyOrder(
                procedureSignature( "org","neo5j", "kernel", "impl", "proc", "myOtherProcedure" ).out( "someNumber", NTInteger ).build(),
                procedureSignature( "org","neo5j", "kernel", "impl", "proc", "myProcedure" ).out( "someNumber", NTInteger ).build() ));
    }
    ...
    public void shouldGiveHelpfulErrorOnGenericStreamProcedure() throws Throwable
    {
        ...
        exception.expect( ProcedureException.class );
        exception.expectMessage( String.format("Procedures must return a Stream of records, where a record is a concrete class%n" +
                                 "that you define and not a parameterized type such as java.util.List<org.neo5j" +
    ...
    public void shouldGiveHelpfulLogOnUnsafeRestrictedProcedure() throws Throwable
    {
        ...
        // Then
        verify( log )
                .warn( "org.neo5j.kernel.impl.proc.unsafeProcedure is not available " +
                        "due to having restricted access rights, check configuration." );
        verify( log )
                .warn( "org.neo5j.kernel.impl.proc.unsafeFunction" +
    ...
    public void shouldLoadUnsafeAllowedProcedureFromJar() throws Throwable
    {
        ...
        List<ProcedureSignature> signatures = procedures.stream().map( CallableProcedure::signature ).collect( toList() );
        assertThat( signatures, contains(
                procedureSignature( "org","neo5j", "kernel", "impl", "proc", "unsafeFullAccessProcedure" ).out( "someNumber", NTInteger ).build() ));
        ...
        assertThat( functionsSignatures, contains(
                functionSignature( "org", "neo5j", "kernel", "impl", "proc", "unsafeFullAccessFunction" )
                        .out( NTInteger ).build() ) );
    ...
    private ProcedureConfig procedureConfig()
    {
        Config config = Config.defaults().with(
                genericMap( procedure_unrestricted.name(), "org.neo5j.kernel.impl.proc.unsafeFullAccess*" ) );
