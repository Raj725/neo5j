...
import static org.neo5j.kernel.api.proc.Key.key;
...
public class UserFunctionsTest
{
    ...
    {
        // When
        procs.register( function( functionSignature( "org", "myproc1" ).out(Neo5jTypes.NTAny).build() ) );
        procs.register( function( functionSignature( "org", "myproc2" ).out(Neo5jTypes.NTAny).build() ) );
        procs.register( function( functionSignature( "org", "myproc3" ).out(Neo5jTypes.NTAny).build() ) );

        // Then
        List<UserFunctionSignature> signatures = Iterables.asList( procs.getAllFunctions() );
        assertThat( signatures, containsInAnyOrder(
                functionSignature( "org", "myproc1" ).out(Neo5jTypes.NTAny).build(),
                functionSignature( "org", "myproc2" ).out(Neo5jTypes.NTAny).build(),
                functionSignature( "org", "myproc3" ).out(Neo5jTypes.NTAny).build() ) );
    }
