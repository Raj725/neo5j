 ...
 */
package org.neo5j.kernel.impl.security;
...
public class FileURLAccessRuleTest
{
    ...
    {
        File importDir = new File( "/tmp/neo5jtest" ).getAbsoluteFile();
    ...
    public void shouldAdjustURLToWithinImportDirectory() throws Exception
    {
        ...
        final Config config = Config.embeddedDefaults(
                MapUtil.stringMap( GraphDatabaseSettings.load_csv_file_url_root.name(), "/var/lib/neo5j/import" ) );
        URL accessURL = URLAccessRules.fileAccess().validate( config, url );
        URL expected = new File( "/var/lib/neo5j/import/bar/baz.csv" ).toURI().toURL();
