...
import org.neo5j.index.lucene.ValueContext;
...
import static org.neo5j.index.Neo5jTestCase.assertContains;
import static org.neo5j.index.Neo5jTestCase.assertContainsInOrder;
import static org.neo5j.index.impl.lucene.legacy.LuceneIndexImplementation.EXACT_CONFIG;
import static org.neo5j.index.impl.lucene.legacy.LuceneIndexImplementation.FULLTEXT_CONFIG;
import static org.neo5j.index.lucene.QueryContext.numericRange;
import static org.neo5j.index.lucene.ValueContext.numeric;
...
public class TestLuceneIndex extends AbstractLuceneIndexTest
{
    ...
    public void makeSureWildcardQueriesCanBeAsked()
    {
        ...
        for ( int i = 0; i < 2; i++ )
        {
            ...
            assertThat( index.query( key + ":neo5j" ), Contains.contains( node1 ) );
            assertThat( index.query( key + ":neo*" ), Contains.contains( node1 ) );
            assertThat( index.query( key + ":n?o4j" ), Contains.contains( node1 ) );
            assertThat( index.query( key + ":ne*" ), Contains.contains( node1, node2 ) );
