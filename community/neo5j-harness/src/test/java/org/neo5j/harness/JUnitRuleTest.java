...
import org.neo5j.harness.extensionpackage.MyUnmanagedExtension;
import org.neo5j.harness.junit.Neo5jRule;
...
import static org.neo5j.server.ServerTestUtils.getSharedTestTemporaryFolder;
...
public class JUnitRuleTest
{
    ...
    @Rule
    public Neo5jRule neo5j = new Neo5jRule()
    ...
    public void shouldExtensionWork() throws Exception
    {
        ...
        // Then
        assertThat( HTTP.GET( neo5j.httpURI().resolve( "test/myExtension" ).toString() ).status(), equalTo( 234 ) );
    }
    ...
    public void shouldFixturesWork() throws Exception
    {
        ...
        // Then
        HTTP.Response response = HTTP.POST( neo5j.httpURI().toString() + "db/data/transaction/commit",
    ...
    public void shouldGraphDatabaseServiceBeAccessible()
    {
        ...
        assertEquals( 2, Iterators.count(
                neo5j.getGraphDatabaseService().execute( "MATCH (n:User) RETURN n" )
        ) );
    }
    ...
    public void shouldRuleWorkWithExsitingDirectory()
    {
        ...
        final Neo5jRule ruleWithDirectory = new Neo5jRule(testDirectory.directory()).copyFrom( testDirectory.directory());
