 ...
 */
package org.neo5j.tooling.procedure;
...
import org.neo5j.tooling.procedure.testutils.JavaFileObjectUtils;
...
public class ProcedureProcessorTest
{
    ...
    public void fails_if_parameters_are_not_properly_annotated()
    {
        ...
        compilation.withErrorContaining( "@org.neo5j.procedure.Name usage error: missing on parameter <parameter>" )
                .in( sproc ).onLine( 35 );

        compilation.withErrorContaining( "@org.neo5j.procedure.Name usage error: missing on parameter <otherParam>" )
                .in( sproc ).onLine( 35 );
    }
    ...
    public void fails_if_duplicate_procedures_are_declared()
    {
        ...
        assert_().about( javaSources() ).that( asList( firstDuplicate, secondDuplicate ) ).processedWith( processor )
                ...
                "Procedure|function name <org.neo5j.tooling.procedure.procedures.invalid.duplicated.foobar> is " +
    ...
    public void fails_if_procedure_class_has_no_public_no_arg_constructor()
    {
        ...
        assert_().about( javaSource() ).that( procedure ).processedWith( processor ).failsToCompile()
                ...
                "Procedure class org.neo5j.tooling.procedure.procedures.invalid.missing_constructor.MissingConstructorProcedure should contain a public no-arg constructor, none found." )
    ...
    public void fails_if_context_injected_fields_have_wrong_modifiers()
    {
        ...
        unsuccessfulCompilationClause.withErrorContaining(
                "@org.neo5j.procedure.Context usage error: field BadContextSproc#shouldBeNonStatic should be public, non-static and non-final" )
                .in( sproc ).onLine( 30 );

        unsuccessfulCompilationClause.withErrorContaining(
                "@org.neo5j.procedure.Context usage error: field BadContextSproc#shouldBeNonFinal should be public, non-static and non-final" )
                .in( sproc ).onLine( 33 );

        unsuccessfulCompilationClause.withErrorContaining(
                "@org.neo5j.procedure.Context usage error: field BadContextSproc#shouldBePublic should be public, non-static and non-final" )
                .in( sproc ).onLine( 37 );
    ...
    public void emits_warnings_if_context_injected_field_types_are_unsupported()
    {
        ...
        assert_().about( javaSource() ).that( sproc ).processedWith( processor ).compilesWithoutError()
                ...
                "@org.neo5j.procedure.Context usage warning: found type: <org.neo5j.kernel.internal.GraphDatabaseAPI>, expected one of: <org.neo5j.graphdb.GraphDatabaseService>, <org.neo5j.logging.Log>" )
                .in( sproc ).onLine( 30 );
    }
