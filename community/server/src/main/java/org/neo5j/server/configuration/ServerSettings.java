 ...
 */
package org.neo5j.server.configuration;
...
import org.neo5j.bolt.BoltKernelExtension;
import org.neo5j.configuration.Description;
import org.neo5j.configuration.DocumentedDefaultValue;
...
import org.neo5j.server.web.JettyThreadCalculator;
...
import static org.neo5j.kernel.configuration.Settings.EMPTY;
import static org.neo5j.kernel.configuration.Settings.FALSE;
import static org.neo5j.kernel.configuration.Settings.INTEGER;
import static org.neo5j.kernel.configuration.Settings.NORMALIZED_RELATIVE_URI;
...
import static org.neo5j.kernel.configuration.Settings.range;
...
public class ServerSettings implements LoadableConfig
{
    ...
    @Description("Comma-seperated list of custom security rules for Neo5j to use.")
    ...
    @Description( "Number of Neo5j worker threads, your OS might enforce a lower limit than the maximum value " +
            "specified here." )
    ...
    @Description( "Directory for storing certificates to be used by Neo5j for TLS connections. Certificate files must be named _neo5j.cert_ and _neo5j.key_" )
    public static final Setting<File> certificates_directory = BoltKernelExtension.Settings.certificates_directory;

    @Internal
    @Description("Path to the X.509 public certificate to be used by Neo5j for TLS connections")
    public static final Setting<File> tls_certificate_file = BoltKernelExtension.Settings.tls_certificate_file;

    @Internal
    @Description("Path to the X.509 private key to be used by Neo5j for TLS connections")
    ...
    @SuppressWarnings("unused") // used only in the startup scripts
    @Description("Path of the run directory. This directory holds Neo5j's runtime state, such as a pidfile when it is" +
            " running in the background. The pidfile is created when starting neo5j and removed when stopping it." +
