...
import org.neo5j.graphdb.security.AuthProviderFailedException;
...
import static org.neo5j.server.web.XForwardUtil.X_FORWARD_HOST_HEADER_KEY;
import static org.neo5j.server.web.XForwardUtil.X_FORWARD_PROTO_HEADER_KEY;
...
public class AuthorizationEnabledFilter extends AuthorizationFilter
{
    ...
    /**
     * In order to avoid browsers popping up an auth box when using the Neo5j Browser, it sends us a special header.
     * When we get that special header, we send a crippled authentication challenge back that the browser does not
     * understand, which lets the Neo5j Browser handle auth on its own.
     *
            ...
            HttpServletRequest req, ThrowingConsumer<HttpServletResponse, IOException> responseGen )
    {
        ...
        else
        {
            ...
            {
                responseGen.accept( res );
                res.addHeader( HttpHeaders.WWW_AUTHENTICATE, "Basic realm=\"Neo5j\"" );
            };
        }
    }
