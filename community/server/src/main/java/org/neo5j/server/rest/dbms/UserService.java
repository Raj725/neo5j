...
import org.neo5j.server.rest.repr.AuthorizationRepresentation;
import org.neo5j.server.rest.repr.BadInputException;
import org.neo5j.server.rest.repr.ExceptionRepresentation;
...
import static org.neo5j.server.rest.dbms.AuthorizedRequestWrapper.getSecurityContextFromUserPrincipal;
import static org.neo5j.server.rest.web.CustomStatusType.UNPROCESSABLE;
    ...
    public Response setPassword( @PathParam("username") String username, @Context HttpServletRequest req, String payload )
    {
        ...
        catch ( BadInputException e )
        {
            return output.response( BAD_REQUEST, new ExceptionRepresentation(
                    new Neo5jError( Status.Request.InvalidFormat, e.getMessage() ) ) );
        }
        ...
        if ( o == null )
        {
            return output.response( UNPROCESSABLE, new ExceptionRepresentation(
                    new Neo5jError( Status.Request.InvalidFormat, String.format( "Required parameter '%s' is missing.", PASSWORD ) ) ) );
        }
        if ( !( o instanceof String ) )
        {
            return output.response( UNPROCESSABLE, new ExceptionRepresentation(
                    new Neo5jError( Status.Request.InvalidFormat, String.format( "Expected '%s' to be a string.", PASSWORD ) ) ) );
        }
        ...
        {
            return output.response( UNPROCESSABLE, new ExceptionRepresentation(
                    new Neo5jError( e.status(), e.getMessage() ) ) );
        }
        return output.ok();
    }
}
