...
import static org.neo5j.server.rest.domain.JsonHelper.writeValue;
 ...
 * <li>{@link #statementResult(org.neo5j.graphdb.Result, boolean, ResultDataContent...) statementResult}{@code *}</li>
...
public class ExecutionResultSerializer
{
    public ExecutionResultSerializer( OutputStream output, URI baseUri, LogProvider logProvider, TransitionalPeriodTransactionMessContainer container )
    {
        ...
        JSON_FACTORY.setCodec( new Neo5jJsonCodec( container ) );
    ...
    /**
     * Will get called once if any errors occurred, after {@link #statementResult(org.neo5j.graphdb.Result, boolean, ResultDataContent...)}  statementResults}
     ...
     */
    public void errors( Iterable<? extends Neo5jError> errors )
    {
        try
        {
            ensureDocumentOpen();
    ...
    public void finish()
    {
        try
        {
            ...
            {
                errors( Collections.<Neo5jError>emptyList() );
            }
            out.writeEndObject();
            out.flush();
        }
