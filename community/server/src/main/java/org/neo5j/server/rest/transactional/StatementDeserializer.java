...
{
    private static final JsonFactory JSON_FACTORY = new JsonFactory().setCodec( new Neo5jJsonCodec() ).disable( JsonGenerator.Feature.FLUSH_PASSED_TO_STREAM );
    private static final Map<String, Object> NO_PARAMETERS = unmodifiableMap( map() );
    private static final Iterator<Neo5jError> NO_ERRORS = emptyIterator();

    private final JsonParser input;
    private State state;
    private List<Neo5jError> errors = null;
    ...
    protected Statement fetchNextOrNull()
    {
        try
        {
            ...
            switch ( state )
            {
                ...
                case IN_BODY:
                    ...
                    if ( statement == null )
                    {
                        addError( new Neo5jError( Status.Request.InvalidFormat, new DeserializationException( "No statement provided." ) ) );
                        return null;
                    }
        ...
        {
            addError( new Neo5jError( Status.Request.InvalidFormat,
                        new DeserializationException( "Unable to deserialize request", e ) ) );
            return null;
        }
        catch ( IOException e )
        {
            addError( new Neo5jError( Status.Network.CommunicationError, e ) );
            return null;
        }
        catch ( Exception e)
        {
            addError( new Neo5jError( Status.General.UnknownError, e ) );
            return null;
        }
    }
    ...
    private void addError( Neo5jError error )
    {
        if ( errors == null )
        {
