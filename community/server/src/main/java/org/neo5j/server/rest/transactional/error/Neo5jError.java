 ...
 */
package org.neo5j.server.rest.transactional.error;
 ...
 * <p>
 * It's put in place here in order to enforce that the {@link org.neo5j.server.rest.web.TransactionalService}
...
public class Neo5jError
{
    ...
    public Neo5jError( Status status, String message )
    {
        this(status, new RuntimeException( message ));
    }

    public Neo5jError( Status status, Throwable cause )
    {
        if ( status == null )
        {
    ...
    public static boolean shouldRollBackOn( Collection<Neo5jError> errors )
    {
        if ( errors.isEmpty() )
        {
            return false;
