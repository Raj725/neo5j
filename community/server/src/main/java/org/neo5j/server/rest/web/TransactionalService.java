...
import org.neo5j.server.rest.dbms.AuthorizedRequestWrapper;
import org.neo5j.server.rest.transactional.ExecutionResultSerializer;
import org.neo5j.server.rest.transactional.TransactionFacade;
import org.neo5j.server.rest.transactional.TransactionHandle;
import org.neo5j.server.rest.transactional.TransactionTerminationHandle;
import org.neo5j.server.rest.transactional.error.Neo5jError;
import org.neo5j.server.rest.transactional.error.TransactionLifecycleException;
import org.neo5j.server.web.HttpHeaderUtils;
import org.neo5j.udc.UsageData;

import static org.neo5j.udc.UsageDataKeys.Features.http_tx_endpoint;
import static org.neo5j.udc.UsageDataKeys.features;

/**
 * This does basic mapping from HTTP to {@link org.neo5j.server.rest.transactional.TransactionFacade}, and should not
...
public class TransactionalService
{
    ...
    private Response invalidTransaction( final TransactionLifecycleException e, final URI baseUri )
    {
        return Response.status( Response.Status.NOT_FOUND )
                .entity( serializeError( e.toNeo5jError(), baseUri ) )
                .build();
    }
    ...
    private StreamingOutput serializeError( final Neo5jError neo5jError, final URI baseUri )
    {
        return output ->
        {
            ExecutionResultSerializer serializer = facade.serializer( output, baseUri );
            serializer.errors( Collections.singletonList( neo5jError ) );
            serializer.finish();
        };
    }
