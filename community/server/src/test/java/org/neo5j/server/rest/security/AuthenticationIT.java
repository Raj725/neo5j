...
import org.neo5j.server.rest.RESTRequestGenerator;
import org.neo5j.server.rest.domain.JsonHelper;
import org.neo5j.server.rest.domain.JsonParseException;
import org.neo5j.test.TestData;
import org.neo5j.test.server.HTTP;
import org.neo5j.test.server.HTTP.RawPayload;
...
public class AuthenticationIT extends CommunityServerTestBase
{
    ...
    public void missing_authorization() throws JsonParseException, IOException
    {
        ...
        RESTRequestGenerator.ResponseEntity response = gen.get()
                .expectedStatus( 401 )
                .expectedHeader( "WWW-Authenticate", "Basic realm=\"Neo5j\"" )
                .get( dataURL() );
    ...
    @Documented( "Authenticate to access the server\n" +
                 "\n" +
                 "Authenticate by sending a username and a password to Neo5j using HTTP Basic Auth.\n" +
    ...
    public void successful_authentication() throws JsonParseException, IOException
    {
        ...
        RESTRequestGenerator.ResponseEntity response = gen.get()
                .expectedStatus( 200 )
                .withHeader( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "secret" ) )
                .get( userURL( "neo5j" ) );

        // Then
        JsonNode data = JsonHelper.jsonNode( response.entity() );
        assertThat( data.get( "username" ).asText(), equalTo( "neo5j" ) );
        assertThat( data.get( "password_change_required" ).asBoolean(), equalTo( false ) );
        assertThat( data.get( "password_change" ).asText(), equalTo( passwordURL( "neo5j" ) ) );
    }
    ...
    public void incorrect_authentication() throws JsonParseException, IOException
    {
        ...
        RESTRequestGenerator.ResponseEntity response = gen.get()
                .expectedStatus( 401 )
                .withHeader( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "incorrect" ) )
                .expectedHeader( "WWW-Authenticate", "Basic realm=\"Neo5j\"" )
                .post( dataURL() );
    ...
    @Documented( "Required password changes\n" +
                 "\n" +
                 "In some cases, like the very first time Neo5j is accessed, the user will be required to choose\n" +
    ...
    public void password_change_required() throws JsonParseException, IOException
    {
        ...
        RESTRequestGenerator.ResponseEntity response = gen.get()
                .expectedStatus( 403 )
                .withHeader( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "neo5j" ) )
                .get( dataURL() );
    ...
    public void shouldReplyNicelyToTooManyFailedAuthAttempts() throws Exception
    {
        ...
        while ( System.currentTimeMillis() < timeout )
        {
            ...
            response = HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "incorrect" ) ).POST(
                    server.baseUri().resolve( "authentication" ).toString(),
                    HTTP.RawPayload.quotedJson( "{'username':'neo5j', 'password':'something that is wrong'}" )
            );
    ...
    public void shouldNotAllowDataAccessForUnauthorizedUser() throws Exception
    {
        ...
        HTTP.Response response =
                HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "neo5j" ) ).POST(
                        server.baseUri().resolve( "authentication" ).toString(),
                        HTTP.RawPayload.quotedJson( "{'username':'neo5j', 'password':'neo5j'}" )
                );

        // When & then
        assertEquals( 403, HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "neo5j" ) )
    ...
    private void assertAuthorizationRequired( String method, String path, Object payload, int expectedAuthorizedStatus ) throws JsonParseException
    {
        ...
        assertThat(response.header( HttpHeaders.WWW_AUTHENTICATE ), equalTo("Basic realm=\"Neo5j\""));
        ...
        response = HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "incorrect" ) ).request( method, server.baseUri().resolve( path ).toString(), payload );
        assertThat(response.status(), equalTo(401));
        assertThat(response.get("errors").get(0).get("code").asText(), equalTo("Neo.ClientError.Security.Unauthorized"));
        ...
        // When authorized
        response = HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "secret" ) ).request( method, server.baseUri().resolve( path ).toString(), payload );
    ...
    public void startServerWithConfiguredUser() throws IOException
    {
        startServer( true );
        // Set the password
        HTTP.Response post = HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "neo5j" ) ).POST(
                server.baseUri().resolve( "/user/neo5j/password" ).toString(),
