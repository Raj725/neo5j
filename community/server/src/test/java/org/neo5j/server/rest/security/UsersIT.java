...
public class UsersIT extends ExclusiveServerTestBase
{
    ...
    public void change_password() throws JsonParseException, IOException
    {
        ...
        RESTRequestGenerator.ResponseEntity response = gen.get()
                ...
                .post( server.baseUri().resolve( "/user/neo5j/password" ).toString() );

        // Then the new password should work
        assertEquals( 200, HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "secret" ) ).GET( dataURL() ).status() );

        // Then the old password should not be invalid
        assertEquals( 401, HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "neo5j" ) ).POST( dataURL() ).status() );
    }
    ...
    public void cantChangeToCurrentPassword() throws Exception
    {
        ...
        // When
        HTTP.Response res = HTTP.withHeaders( HttpHeaders.AUTHORIZATION, challengeResponse( "neo5j", "neo5j" ) ).POST(
                server.baseUri().resolve( "/user/neo5j/password" ).toString(),
                HTTP.RawPayload.quotedJson( "{'password':'neo5j'}" ) );
