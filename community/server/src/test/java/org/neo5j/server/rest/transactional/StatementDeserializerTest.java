...
public class StatementDeserializerTest
{
    ...
    public void shouldRejectMapWithADifferentFieldBeforeStatement() throws Exception
    {
        ...
        assertYieldsErrors( json,
                new Neo5jError( Status.Request.InvalidFormat,
    ...
    {
        assertYieldsErrors( "{}",
                new Neo5jError( Status.Request.InvalidFormat, new DeserializationException( "Unable to " +
                        "deserialize request. " +
    ...
    private void assertYieldsErrors( String json, Neo5jError... expectedErrors ) throws UnsupportedEncodingException
    {
        StatementDeserializer de = new StatementDeserializer( new ByteArrayInputStream( UTF8.encode( json ) )
        {
            @Override
        ...
        Iterator<Neo5jError> actual = de.errors();
        Iterator<Neo5jError> expected = asList( expectedErrors ).iterator();
        while ( actual.hasNext() )
        {
            assertTrue( expected.hasNext() );
            Neo5jError error = actual.next();
            Neo5jError expectedError = expected.next();
