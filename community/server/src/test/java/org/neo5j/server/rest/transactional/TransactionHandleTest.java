...
import static org.neo5j.server.rest.transactional.StubStatementDeserializer.statements;
...
public class TransactionHandleTest
{
    ...
    private static Matcher<Iterable<Neo5jError>> hasNoErrors()
    {
        return hasErrors();
    }

    private static Matcher<Iterable<Neo5jError>> hasErrors( Status... codes )
    {
        final Set<Status> expectedErrorsCodes = new HashSet<>( asList( codes ) );

        return new TypeSafeMatcher<Iterable<Neo5jError>>()
        {
            @Override
            protected boolean matchesSafely( Iterable<Neo5jError> item )
            {
                Set<Status> actualErrorCodes = new HashSet<>();
                for ( Neo5jError neo5jError : item )
                {
                    actualErrorCodes.add( neo5jError.status() );
                }
