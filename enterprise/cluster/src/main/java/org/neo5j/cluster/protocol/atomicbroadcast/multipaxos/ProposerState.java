...
import org.neo5j.cluster.statemachine.State;
        ...
        implements State<ProposerContext, ProposerMessage>
{
                        ...
                        throws Throwable
                {
                    switch ( message.getMessageType() )
                    {
                        ...
                        case rejectPrepare:
                        {
                            ...
                            org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId instanceId = new org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId( message );
                                    ...
                                    instance.getBallot() < rejectPropose.getBallot()) // Ignore multiple rejects on same prepare
                            {
                                ...
                                for ( URI acceptor : instance.getAcceptors() )
                                {
                                    outgoing.offer( message.copyHeadersTo( Message.to( AcceptorMessage.prepare,
                                            ...
                                            org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE ) );
                                }
                                ...
                                context.setTimeout( instanceId, message.copyHeadersTo( Message.timeout(
                                        ProposerMessage.phase1Timeout, message, payload ), org.neo5j.cluster.protocol
                        ...
                        case promise:
                        {
                            ...
                            PaxosInstance instance = context.getPaxosInstance( new org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId( message ) );
                                    ...
                                    for ( URI acceptor : instance.getAcceptors() )
                                    {
                                        outgoing.offer( message.copyHeadersTo( Message.to( AcceptorMessage.accept,
                                                ...
                                                new AcceptorMessage.AcceptState( instance.ballot,
                                                        instance.value_2 ) ), org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE ) );
                                    }

                                    context.setTimeout( instance.id,
                                            message.copyHeadersTo( Message.timeout( ProposerMessage.phase2Timeout,
                                                    message, readyValue ), org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE ) );
                                }
                                else
                                {
                                }
                        ...
                        case accepted:
                        {
                            PaxosInstance instance = context.getPaxosInstance( new org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId(
                                    message ) );
    ...
    private static void propose( ProposerContext context, Message message, MessageHolder outgoing,
                                 List<URI> acceptors )
    {
        org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId instanceId;
        if ( message.hasHeader( org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE ) )
        {
            instanceId = new org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId( message );
        }
        else
        {
        ...
        if ( !(instance.isState( PaxosInstance.State.closed ) || instance.isState( PaxosInstance.State.delivered )) )
        {
            ...
            for ( URI acceptor : acceptors )
            {
                outgoing.offer( Message.to( AcceptorMessage.prepare, acceptor, new AcceptorMessage.PrepareState(
                        ballot ) ).setHeader( org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE, instanceId.toString() ) );
            }

            context.setTimeout( instanceId, Message.timeout( ProposerMessage.phase1Timeout, message,
                    message.getPayload() ).setHeader( org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE, instanceId.toString() ) );
        }
        else
        {
    ...
    private static List<URI> determineAcceptorSet( Message<ProposerMessage> message, ProposerContext context )
    {
        ...
        if ( payload instanceof ClusterMessage.ConfigurationChangeState )
        {
            ...
            Map<org.neo5j.cluster.InstanceId, URI> currentMembers = context.getMembers();
