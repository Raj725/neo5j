 ...
 */
package org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.context;
...
import org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.DefaultWinnerStrategy;
import org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.Vote;
import org.neo5j.cluster.protocol.atomicbroadcast.multipaxos.WinnerStrategy;
import org.neo5j.cluster.protocol.cluster.ClusterContext;
import org.neo5j.cluster.protocol.cluster.ClusterMessage;
import org.neo5j.cluster.protocol.election.ElectionContext;
import org.neo5j.cluster.protocol.election.ElectionCredentials;
...
import org.neo5j.cluster.protocol.election.NotElectableElectionCredentials;
import org.neo5j.cluster.protocol.heartbeat.HeartbeatContext;
import org.neo5j.cluster.protocol.heartbeat.HeartbeatListener;
import org.neo5j.cluster.timeout.Timeouts;
import org.neo5j.logging.LogProvider;

import static org.neo5j.cluster.util.Quorums.isQuorum;
        ...
        implements ElectionContext, HeartbeatListener
{
    ...
    ElectionContextImpl( org.neo5j.cluster.InstanceId me, CommonContextState commonState,
                         LogProvider logging,
     ...
     */
    @Override
    public void nodeFailed( org.neo5j.cluster.InstanceId node )
    {
    ...
    @Override
    public Iterable<String> getRoles( org.neo5j.cluster.InstanceId server )
    {
    ...
    @Override
    public boolean voted( String role, org.neo5j.cluster.InstanceId suggestedNode,
                          ElectionCredentials suggestionCredentials, long electionVersion )
    {
        ...
        Map<org.neo5j.cluster.InstanceId, Vote> votes = elections.get( role ).getVotes();
        votes.put( suggestedNode, new Vote( suggestedNode, suggestionCredentials ) );
        return true;
    }

    @Override
    public org.neo5j.cluster.InstanceId getElectionWinner( String role )
    {
    ...
    public int getVoteCount( String role )
    {
        ...
        if ( election != null )
        {
            Map<org.neo5j.cluster.InstanceId, Vote> voteList = election.getVotes();
            if ( voteList == null )
            {
                return 0;
            }
    ...
    @Override
    public Iterable<org.neo5j.cluster.InstanceId> getAlive()
    {
        return heartbeatContext.getAlive();
    }

    @Override
    public org.neo5j.cluster.InstanceId getMyId()
    {
    ...
    public boolean isElector()
    {
        ...
        List<org.neo5j.cluster.InstanceId> aliveInstances = asList( getAlive() );
        Collections.sort( aliveInstances );
        return aliveInstances.indexOf( getMyId() ) == 0;
    }

    @Override
    public boolean isFailed( org.neo5j.cluster.InstanceId key )
    {
        return heartbeatContext.getFailed().contains( key );
    }

    @Override
    public org.neo5j.cluster.InstanceId getElected( String roleName )
    {
        return clusterContext.getConfiguration().getElected( roleName );
    }

    @Override
    public boolean hasCurrentlyElectedVoted( String role, org.neo5j.cluster.InstanceId currentElected )
    {
    ...
    private static class Election
    {
        ...
        private final Map<org.neo5j.cluster.InstanceId, Vote> votes;
    ...
    @Override
    public void failed( org.neo5j.cluster.InstanceId server )
    {
    ...
    @Override
    public void alive( org.neo5j.cluster.InstanceId server )
    {
        // Not needed
    }
