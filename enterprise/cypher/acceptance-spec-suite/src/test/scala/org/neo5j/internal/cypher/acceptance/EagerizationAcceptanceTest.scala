...
import org.neo5j.cypher.internal.compiler.v3_2.helpers.Counter
import org.neo5j.cypher.internal.compiler.v3_2.test_helpers.CreateTempFileTestSupport
import org.neo5j.cypher.{ExecutionEngineFunSuite, NewPlannerTestSupport, QueryStatisticsTestSupport}
import org.neo5j.graphdb.{Direction, Node}
import org.neo5j.kernel.api.exceptions.ProcedureException
import org.neo5j.kernel.api.proc
...
class EagerizationAcceptanceTest
  ...
  test("should not introduce extra eagerness after CALL of writing procedure") {
    ...
    registerProcedure("user.mkRel") { builder =>
      builder.in("x", Neo5jTypes.NTNode)
      builder.in("y", Neo5jTypes.NTNode)
      builder.out("relId", Neo5jTypes.NTInteger)
  ...
  test("should not introduce extra eagerness after CALL of writing void procedure") {
    ...
    registerProcedure("user.mkRel") { builder =>
      builder.in("x", Neo5jTypes.NTNode)
      builder.in("y", Neo5jTypes.NTNode)
      builder.out(org.neo5j.kernel.api.proc.ProcedureSignature.VOID)
      builder.mode(Mode.WRITE)
      new BasicProcedure(builder.build) {
        override def apply(ctx: Context, input: Array[AnyRef]): RawIterator[Array[AnyRef], ProcedureException] = {
          val transaction = ctx.get(proc.Context.KERNEL_TRANSACTION)
