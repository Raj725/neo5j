...
class SetAcceptanceTest extends ExecutionEngineFunSuite with QueryStatisticsTestSupport with NewPlannerTestSupport {
  ...
  test("should not be able to set property to collection of collections") {
    ...
    //then
    result.toString should equal("org.neo5j.cypher.CypherTypeException: Collections containing collections can not be stored in properties.")
  }
  ...
  test("should not be able to set property to collection with null value") {
    ...
    //then
    result.toString should equal("org.neo5j.cypher.CypherTypeException: Collections containing null values can not be stored in properties.")
  }
  ...
  test("set a property by selecting the node through an expression") {
    ...
    // when
    val result = updateWithBothPlannersAndCompatibilityMode("MATCH (n) SET (CASE WHEN true THEN n END).name = 'neo5j' RETURN count(*)")

    // then
    assertStats(result, propertiesWritten = 1)
    a should haveProperty("name").withValue("neo5j")
  }
  ...
  test("set a property by selecting the relationship through an expression") {
    ...
    // when
    val result = updateWithBothPlannersAndCompatibilityMode("MATCH ()-[r]->() SET (CASE WHEN true THEN r END).name = 'neo5j' RETURN count(*)")

    // then
    assertStats(result, propertiesWritten = 1)
    r should haveProperty("name").withValue("neo5j")
  }
  ...
  test("should fail at runtime when the expression is not a node or a relationship") {
    an [InvalidArgumentException] should be thrownBy
      updateWithBothPlanners("SET (CASE WHEN true THEN {node} END).name = 'neo5j' RETURN count(*)", "node" -> 42)
  }
