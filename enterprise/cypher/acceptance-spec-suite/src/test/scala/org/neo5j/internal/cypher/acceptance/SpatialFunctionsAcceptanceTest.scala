...
import org.neo5j.cypher.internal.compiler.v3_2.{CRS, CartesianPoint, GeographicPoint}
import org.neo5j.cypher.{ExecutionEngineFunSuite, InvalidArgumentException, NewPlannerTestSupport, SyntaxException}
...
class SpatialFunctionsAcceptanceTest extends ExecutionEngineFunSuite with NewPlannerTestSupport {
  ...
  test("distance function should measure distance from Copenhagen train station to Neo5j in Malmö") {
    val result = executeWithAllPlanners(
      """
