...
import org.neo5j.cypher.internal.{ExecutionEngine, ExecutionResult}
...
import org.neo5j.graphdb.Result.{ResultRow, ResultVisitor}
...
import org.neo5j.kernel.api.proc.Context.KERNEL_TRANSACTION
import org.neo5j.kernel.api.proc._
...
class CloseTransactionTest extends CypherFunSuite with GraphIcing {
  ...
  for (runtime <- runtimes) {
    ...
    test(s"should not leak transaction when closing the result for a procedure query - runtime=$runtime") {
      ...
      // when
      db.execute(s"CYPHER runtime=$runtime CALL org.neo5j.bench.getAllNodes()").close()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).close()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).javaIterator.close()
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when closing the result for a profile procedure query - runtime=$runtime") {
      ...
      // when
      db.execute(s"CYPHER runtime=$runtime profile CALL org.neo5j.bench.getAllNodes()").close()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime profile CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).close()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime profile CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).javaIterator.close()
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when closing the result for an explain procedure query - runtime=$runtime") {
      ...
      // when
      db.execute(s"CYPHER runtime=$runtime explain CALL org.neo5j.bench.getAllNodes()").close()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime explain CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).close()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime explain CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).javaIterator.close()
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when consuming the whole iterator for a procedure query - runtime=$runtime") {
      ...
      import scala.collection.JavaConverters._
      // when
      db.execute(s"CYPHER runtime=$runtime CALL org.neo5j.bench.getAllNodes()").asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).length
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).javaIterator.asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when consuming the whole iterator for a profile procedure query - runtime=$runtime") {
      ...
      import scala.collection.JavaConverters._
      // when
      db.execute(s"CYPHER runtime=$runtime profile CALL org.neo5j.bench.getAllNodes()").asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime profile CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).length
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime profile CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).javaIterator.asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when consuming the whole iterator for an explain procedure query - runtime=$runtime") {
      ...
      import scala.collection.JavaConverters._
      // when
      db.execute(s"CYPHER runtime=$runtime explain CALL org.neo5j.bench.getAllNodes()").asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime explain CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).length
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime explain CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).javaIterator.asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when visiting the result for a procedure query - runtime=$runtime") {
      ...
      db.execute(s"CYPHER runtime=$runtime CALL org.neo5j.bench.getAllNodes()").accept(consumerVisitor)

      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).accept(consumerVisitor)
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when visiting the result for a profile procedure query - runtime=$runtime") {
      ...
      db.execute(s"CYPHER runtime=$runtime profile CALL org.neo5j.bench.getAllNodes()").accept(consumerVisitor)

      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime profile CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).accept(consumerVisitor)
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when visiting the result for an explain procedure query - runtime=$runtime") {
      ...
      db.execute(s"CYPHER runtime=$runtime explain CALL org.neo5j.bench.getAllNodes()").accept(consumerVisitor)

      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime explain CALL org.neo5j.bench.getAllNodes()", Map.empty[String, Object]).accept(consumerVisitor)
      // then
      txBridge(service).hasTransaction shouldBe false
    }
  }
  ...
  class AllNodesProcedure extends CallableProcedure {
    ...
    private val results = Map[String, AnyRef]("node" -> Neo5jTypes.NTInteger)
    val procedureName = new QualifiedName(Array[String]("org", "neo5j", "bench"), "getAllNodes")
    ...
    def resultSignature: util.List[FieldSignature] = results.keys.foldLeft(List.empty[FieldSignature]) { (fields, entry) =>
      fields :+ new FieldSignature(entry, results(entry).asInstanceOf[Neo5jTypes.AnyType])
    }.asJava
