...
class MutatingIntegrationTest extends ExecutionEngineFunSuite with Assertions with QueryStatisticsTestSupport with NewPlannerTestSupport {
  ...
  test("fail to create from two iterables") {
    ...
    try {
      ...
      case e: CypherTypeException => e.getCause shouldBe a [org.neo5j.cypher.internal.frontend.v3_2.CypherTypeException]
      case e: ParameterWrongTypeException => e.getCause shouldBe a [org.neo5j.cypher.internal.frontend.v2_3.ParameterWrongTypeException]
    }
  }
  ...
  test("first read then write") {
    ...
    val result = executeWithAllPlannersAndRuntimesAndCompatibilityMode("match (root) where id(root) = 0 match (root)-->(other) return other.name order by other.name").columnAs[String]("other.name").toList
  ...
  test("string literals should not be mistaken for variables") {
    //https://github.com/neo5j/community/issues/523
