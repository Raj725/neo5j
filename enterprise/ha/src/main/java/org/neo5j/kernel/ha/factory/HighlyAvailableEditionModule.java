 ...
 */
package org.neo5j.kernel.ha.factory;
...
import org.neo5j.cluster.client.ClusterClient;
import org.neo5j.cluster.client.ClusterClientModule;
import org.neo5j.cluster.logging.NettyLoggerFactory;
import org.neo5j.cluster.member.ClusterMemberAvailability;
import org.neo5j.cluster.member.ClusterMemberEvents;
import org.neo5j.cluster.member.paxos.MemberIsAvailable;
import org.neo5j.cluster.member.paxos.PaxosClusterMemberAvailability;
import org.neo5j.cluster.member.paxos.PaxosClusterMemberEvents;
import org.neo5j.cluster.protocol.atomicbroadcast.ObjectStreamFactory;
import org.neo5j.cluster.protocol.cluster.ClusterConfiguration;
import org.neo5j.cluster.protocol.cluster.ClusterListener;
import org.neo5j.cluster.protocol.election.ElectionCredentialsProvider;
import org.neo5j.cluster.protocol.election.NotElectableElectionCredentialsProvider;
import org.neo5j.com.Server;
import org.neo5j.com.monitor.RequestMonitor;
import org.neo5j.com.storecopy.StoreCopyClient;
import org.neo5j.com.storecopy.StoreUtil;
import org.neo5j.com.storecopy.TransactionCommittingResponseUnpacker;
...
import org.neo5j.helpers.NamedThreadFactory;
...
import org.neo5j.kernel.ha.BranchDetectingTxVerifier;
import org.neo5j.kernel.ha.BranchedDataMigrator;
import org.neo5j.kernel.ha.DelegateInvocationHandler;
import org.neo5j.kernel.ha.HaSettings;
import org.neo5j.kernel.ha.HighAvailabilityDiagnostics;
import org.neo5j.kernel.ha.HighAvailabilityLogger;
import org.neo5j.kernel.ha.LastUpdateTime;
import org.neo5j.kernel.ha.PullerFactory;
import org.neo5j.kernel.ha.TransactionChecksumLookup;
import org.neo5j.kernel.ha.UpdatePuller;
import org.neo5j.kernel.ha.cluster.ConversationSPI;
import org.neo5j.kernel.ha.cluster.DefaultConversationSPI;
import org.neo5j.kernel.ha.cluster.DefaultElectionCredentialsProvider;
import org.neo5j.kernel.ha.cluster.DefaultMasterImplSPI;
import org.neo5j.kernel.ha.cluster.HANewSnapshotFunction;
import org.neo5j.kernel.ha.cluster.HighAvailabilityMemberChangeEvent;
import org.neo5j.kernel.ha.cluster.HighAvailabilityMemberContext;
import org.neo5j.kernel.ha.cluster.HighAvailabilityMemberListener;
import org.neo5j.kernel.ha.cluster.HighAvailabilityMemberState;
import org.neo5j.kernel.ha.cluster.HighAvailabilityMemberStateMachine;
import org.neo5j.kernel.ha.cluster.SimpleHighAvailabilityMemberContext;
import org.neo5j.kernel.ha.cluster.SwitchToMaster;
import org.neo5j.kernel.ha.cluster.SwitchToSlave;
import org.neo5j.kernel.ha.cluster.SwitchToSlaveBranchThenCopy;
import org.neo5j.kernel.ha.cluster.SwitchToSlaveCopyThenBranch;
import org.neo5j.kernel.ha.cluster.member.ClusterMembers;
import org.neo5j.kernel.ha.cluster.member.HighAvailabilitySlaves;
import org.neo5j.kernel.ha.cluster.member.ObservedClusterMembers;
import org.neo5j.kernel.ha.cluster.modeswitch.CommitProcessSwitcher;
import org.neo5j.kernel.ha.cluster.modeswitch.ComponentSwitcherContainer;
import org.neo5j.kernel.ha.cluster.modeswitch.HighAvailabilityModeSwitcher;
import org.neo5j.kernel.ha.cluster.modeswitch.LabelTokenCreatorSwitcher;
import org.neo5j.kernel.ha.cluster.modeswitch.LockManagerSwitcher;
import org.neo5j.kernel.ha.cluster.modeswitch.PropertyKeyCreatorSwitcher;
import org.neo5j.kernel.ha.cluster.modeswitch.RelationshipTypeCreatorSwitcher;
import org.neo5j.kernel.ha.cluster.modeswitch.UpdatePullerSwitcher;
import org.neo5j.kernel.ha.com.RequestContextFactory;
import org.neo5j.kernel.ha.com.master.ConversationManager;
import org.neo5j.kernel.ha.com.master.DefaultSlaveFactory;
import org.neo5j.kernel.ha.com.master.Master;
import org.neo5j.kernel.ha.com.master.MasterImpl;
import org.neo5j.kernel.ha.com.master.MasterServer;
import org.neo5j.kernel.ha.com.master.Slave;
import org.neo5j.kernel.ha.com.master.SlaveFactory;
import org.neo5j.kernel.ha.com.master.Slaves;
import org.neo5j.kernel.ha.com.slave.InvalidEpochExceptionHandler;
import org.neo5j.kernel.ha.com.slave.MasterClientResolver;
import org.neo5j.kernel.ha.com.slave.SlaveServer;
import org.neo5j.kernel.ha.id.HaIdGeneratorFactory;
import org.neo5j.kernel.ha.id.HaIdReuseEligibility;
import org.neo5j.kernel.ha.management.ClusterDatabaseInfoProvider;
import org.neo5j.kernel.ha.management.HighlyAvailableKernelData;
import org.neo5j.kernel.ha.transaction.CommitPusher;
import org.neo5j.kernel.ha.transaction.OnDiskLastTxIdGetter;
...
import org.neo5j.kernel.impl.api.index.RemoveOrphanConstraintIndexesOnStartup;
...
import org.neo5j.kernel.impl.core.LastTxIdGetter;
import org.neo5j.kernel.impl.core.ReadOnlyTokenCreator;
import org.neo5j.kernel.impl.core.TokenCreator;
...
import org.neo5j.kernel.impl.store.TransactionId;
...
import org.neo5j.kernel.impl.transaction.log.NoSuchTransactionException;
...
import static org.neo5j.kernel.impl.transaction.log.TransactionMetadataCache.TransactionMetadata;
