...
public class ClusterMemberInfo implements Serializable
{
    ...
    {
        return String.format( "Neo5jHaInstance[id=%s,available=%s,haRole=%s,HA URI=%s]", instanceId, available, haRole,
                Arrays.toString(uris) );
    }

    public Pair<Neo5jManager, HighAvailability> connect()
    {
        return connect( null, null );
    }

    public Pair<Neo5jManager, HighAvailability> connect( String username, String password )
    {
        URI address = null;
        for (String uri : uris)
        {
...
//                address = uri;
        ...
        Neo5jManager manager = Neo5jManager.get( url(address), username, password, instanceId );
