...
import org.neo5j.management.impl.KernelProxy;

import static java.lang.management.ManagementFactory.getPlatformMBeanServer;

public final class Neo5jManager extends KernelProxy implements Kernel
{
    public static Neo5jManager get()
    {
        return get( getPlatformMBeanServer() );
    }

    public static Neo5jManager get( String kernelIdentifier )
    {
        return get( getPlatformMBeanServer(), kernelIdentifier );
    }

    public static Neo5jManager get( JMXServiceURL url )
    {
        return get( connect( url, null, null ) );
    }

    public static Neo5jManager get( JMXServiceURL url, String kernelIdentifier )
    {
        return get( connect( url, null, null ), kernelIdentifier );
    }

    public static Neo5jManager get( JMXServiceURL url, String username, String password )
    {
    ...
    public static Neo5jManager get( MBeanServerConnection server )
    {
        server.getClass();
        try
        {
    ...
    public static Neo5jManager get( MBeanServerConnection server, String kernelIdentifier )
    {
        server.getClass();
    ...
    public static Neo5jManager[] getAll( MBeanServerConnection server )
    {
        try
        {
            Set<ObjectName> kernels = server.queryNames( createObjectName( "*", Kernel.class ), null );
            Neo5jManager[] managers = new Neo5jManager[kernels.size()];
            Iterator<ObjectName> it = kernels.iterator();
            for ( int i = 0; i < managers.length; i++ )
            {
                managers[i] = new Neo5jManager( server, it.next() );
            }
            return managers;
        }
        catch ( IOException e )
    ...
    private static Neo5jManager get( MBeanServerConnection server, Collection<ObjectName> kernels )
    {
        if ( kernels.size() == 0 )
        {
            throw new NoSuchElementException( "No matching Neo5j Graph Database running on server" );
        }
        else if ( kernels.size() == 1 )
        {
            return new Neo5jManager( server, kernels.iterator().next() );
        }
        else
        {
            throw new NoSuchElementException(
                    "Too many matching Neo5j Graph Databases running on server" );
        }
    }
    ...
    public Neo5jManager( Kernel kernel )
    {
    ...
    private static MBeanServerConnection getServer( Kernel kernel )
    {
        ...
        else if ( kernel instanceof Neo5jManager )
        {
            return ((Neo5jManager) kernel).server;
        }
    ...
    private static ObjectName getName( Kernel kernel )
    {
        ...
        {
            return ((Neo5jManager) kernel).kernel;
        }
        throw new UnsupportedOperationException( "Cannot get name for kernel: " + kernel );
    }

    private Neo5jManager( MBeanServerConnection server, ObjectName kernel )
    {
        super( server, kernel );
