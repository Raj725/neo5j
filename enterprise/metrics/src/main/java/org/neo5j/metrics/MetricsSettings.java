...
/**
 * Settings for the Neo5j Enterprise metrics reporting.
 */
@Description( "Metrics settings" )
public class MetricsSettings implements LoadableConfig
{
    // Common settings
    @Description( "A common prefix for the reported metrics field names. By default, this is either be 'neo5j', " +
                  "or a computed value based on the cluster and instance names, when running in an HA configuration." )
    public static Setting<String> metricsPrefix = setting( "metrics.prefix", Settings.STRING, "neo5j" );
    ...
    @Description( "The default enablement value for all Neo5j specific support metrics. Set this to `false` to turn " +
                  "off all Neo5j specific metrics by default. The individual `metrics.neo5j.*` metrics can then be " +
                  "turned on selectively." )
    public static Setting<Boolean> neoEnabled = setting( "metrics.neo5j.enabled", Settings.BOOLEAN, metricsEnabled );
    @Description( "Enable reporting metrics about transactions; number of transactions started, committed, etc." )
    public static Setting<Boolean> neoTxEnabled = setting( "metrics.neo5j.tx.enabled", Settings.BOOLEAN, neoEnabled );
    @Description( "Enable reporting metrics about the Neo5j page cache; page faults, evictions, flushes, exceptions, " +
                  "etc." )
    public static Setting<Boolean> neoPageCacheEnabled = setting(
            "metrics.neo5j.pagecache.enabled", Settings.BOOLEAN, neoEnabled );
    ...
    public static Setting<Boolean> causalClusteringEnabled = setting(
            "metrics.neo5j.causal_clustering.enabled", Settings.BOOLEAN, neoEnabled );
    @Description( "Enable reporting metrics about Neo5j check pointing; when it occurs and how much time it takes to " +
                  "complete." )
    public static Setting<Boolean> neoCheckPointingEnabled = setting(
            "metrics.neo5j.checkpointing.enabled", Settings.BOOLEAN, neoEnabled );
    @Description( "Enable reporting metrics about the Neo5j log rotation; when it occurs and how much time it takes to "
                  + "complete." )
    public static Setting<Boolean> neoLogRotationEnabled = setting(
            "metrics.neo5j.logrotation.enabled", Settings.BOOLEAN, neoEnabled );
    @Description( "Enable reporting metrics about HA cluster info." )
    public static Setting<Boolean> neoClusterEnabled = setting(
            "metrics.neo5j.cluster.enabled", Settings.BOOLEAN, neoEnabled );
    @Description( "Enable reporting metrics about Server threading info." )
    public static Setting<Boolean> neoServerEnabled = setting(
            "metrics.neo5j.server.enabled", Settings.BOOLEAN, neoEnabled );
