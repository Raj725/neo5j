...
import org.neo5j.server.security.auth.exception.ConcurrentModificationException;
        ...
        ShiroAuthorizationInfoProvider
{
    ...
    /* Adds neo5j user if no users exist */
    ...
    /* Builds all predefined roles if no roles exist. Adds 'neo5j' to admin role if no admin is assigned */
                    ...
                    if ( defaultAdminRepository.numberOfUsers() > 1 )
                    {
                        throw new InvalidArgumentsException(
                                "No roles defined, and multiple users defined as default admin user." +
                                        " Please use `neo5j-admin " + SetDefaultAdminCommand.COMMAND_NAME +
                        ...
                        if ( userRepository.getUserByName( newAdminUsername ) == null )
                        {
                            throw new InvalidArgumentsException(
                                    "No roles defined, and default admin user '" + newAdminUsername +
                                            "' does not exist. Please use `neo5j-admin " +
