...
import org.neo5j.server.security.enterprise.auth.PredefinedRolesBuilder;
import org.neo5j.server.security.enterprise.auth.RealmLifecycle;
import org.neo5j.server.security.enterprise.auth.SecureHasher;
import org.neo5j.server.security.enterprise.auth.ShiroAuthToken;
import org.neo5j.server.security.enterprise.auth.ShiroAuthorizationInfoProvider;
import org.neo5j.server.security.enterprise.auth.plugin.api.AuthProviderOperations;
import org.neo5j.server.security.enterprise.auth.plugin.api.AuthToken;
import org.neo5j.server.security.enterprise.auth.plugin.api.AuthorizationExpiredException;
...
import org.neo5j.server.security.enterprise.auth.plugin.spi.AuthorizationPlugin;
...
import static org.neo5j.server.security.enterprise.configuration.SecuritySettings.PLUGIN_REALM_NAME_PREFIX;
...
public class PluginRealm extends AuthorizingRealm implements RealmLifecycle, ShiroAuthorizationInfoProvider
{
    ...
    protected AuthorizationInfo doGetAuthorizationInfo( PrincipalCollection principals )
    {
        ...
        {
            org.neo5j.server.security.enterprise.auth.plugin.spi.AuthorizationInfo authorizationInfo;
            try
            {
            ...
            {
                throw new org.neo5j.graphdb.security.AuthorizationExpiredException(
    ...
    protected AuthenticationInfo doGetAuthenticationInfo( AuthenticationToken token ) throws AuthenticationException
    {
        if ( token instanceof ShiroAuthToken )
        {
            try
            {
                ...
                {
                    org.neo5j.server.security.enterprise.auth.plugin.spi.AuthenticationInfo authenticationInfo =
            ...
            catch ( org.neo5j.server.security.enterprise.auth.plugin.api.AuthenticationException |
    ...
    private class PluginRealmOperations implements AuthProviderOperations
    {
        ...
        @Override
        public Path neo5jHome()
        {
            return config.get( GraphDatabaseSettings.neo5j_home ).getAbsoluteFile().toPath();
        }

        @Override
        public Optional<Path> neo5jConfigFile()
        {
            return config.getConfigFile();
        }

        @Override
        public String neo5jVersion()
        {
            return Version.getNeo5jVersion();
        }
