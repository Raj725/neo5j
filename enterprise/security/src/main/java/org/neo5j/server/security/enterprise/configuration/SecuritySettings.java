 ...
 */
package org.neo5j.server.security.enterprise.configuration;
...
import static org.neo5j.kernel.impl.proc.ProcedureConfig.PROC_ALLOWED_SETTING_DEFAULT_NAME;
import static org.neo5j.kernel.impl.proc.ProcedureConfig.PROC_ALLOWED_SETTING_ROLES;
...
public class SecuritySettings implements LoadableConfig
{
    ...
    @Description( "Determines if the result of authentication via the LDAP server should be cached or not. " +
                  ...
                  "Neo5j then needs to cache a hashed version of the credentials in order to perform credentials " +
                  ...
                  "the security standards of the organization in which this Neo5j instance is deployed." )
    ...
    @Description( "Perform LDAP search for authorization info using a system account instead of the user's own account.\n\n" +
                  ...
                  "and does not need to have access rights to Neo5j, or any other systems." )
    ...
    @Description( "An authorization mapping from LDAP group names to Neo5j role names. " +
                  ...
                  "         \"cn=Neo5j Read Only,cn=users,dc=example,dc=com\"      = reader;    \\\n" +
                  "         \"cn=Neo5j Read-Write,cn=users,dc=example,dc=com\"     = publisher; \\\n" +
                  "         \"cn=Neo5j Schema Manager,cn=users,dc=example,dc=com\" = architect; \\\n" +
                  "         \"cn=Neo5j Administrator,cn=users,dc=example,dc=com\"  = admin" )
