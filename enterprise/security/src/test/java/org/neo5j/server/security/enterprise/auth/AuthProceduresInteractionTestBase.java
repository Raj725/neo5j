...
import static org.neo5j.kernel.api.security.AuthenticationResult.PASSWORD_CHANGE_REQUIRED;
import static org.neo5j.server.security.enterprise.auth.InternalFlatFileRealm.IS_SUSPENDED;
import static org.neo5j.server.security.enterprise.auth.ProcedureInteractionTestBase.ClassWithProcedures.exceptionsInProcedure;
...
import static org.neo5j.server.security.enterprise.auth.plugin.api.PredefinedRoles.EDITOR;
import static org.neo5j.server.security.enterprise.auth.plugin.api.PredefinedRoles.PUBLISHER;
...
public abstract class AuthProceduresInteractionTestBase<S> extends ProcedureInteractionTestBase<S>
{
    ...
    public void shouldReturnUsersWithFlags() throws Exception
    {
        Map<String,Object> expected = map(
                ...
                "noneSubject", listOf(),
                "neo5j", listOf( PWD_CHANGE.toLowerCase() )
        );
    ...
    {
        Map<String,Object> expected = map(
                ADMIN, listOf( "adminSubject", "neo5j" ),
    ...
    public void shouldListUsersForRole() throws Exception
    {
        assertSuccess( adminSubject, "CALL dbms.security.listUsersForRole('admin') YIELD value as users RETURN users",
                r -> assertKeyIs( r, "users", "adminSubject", "neo5j" ) );
    }
