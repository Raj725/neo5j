...
public abstract class AuthScenariosInteractionTestBase<S> extends ProcedureInteractionTestBase<S>
{
    ...
    public void shouldLogSecurityEvents() throws Exception
    {
        ...
        assertEmpty( adminSubject, "CALL dbms.security.createUser('mats', 'neo5j', false)" );
        ...
        mats = neo.login( "mats", "neo5j" );
        assertEmpty( mats, "MATCH (n) WHERE id(n) < 0 RETURN 1" );
        assertFail( mats, "CALL dbms.security.changeUserPassword('neo5j', 'hackerPassword')", PERMISSION_DENIED );
        ...
        log.assertHasLine( "mats", "tried to change password for user `neo5j`: " + PERMISSION_DENIED);
