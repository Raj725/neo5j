...
import org.neo5j.server.security.auth.ListSnapshot;
import org.neo5j.server.security.auth.RateLimitedAuthenticationStrategy;
...
import static org.neo5j.server.security.enterprise.auth.AuthTestUtil.listOf;
...
public class InternalFlatFileRealmTest
{
    ...
    {
        // Given
        InternalFlatFileRealm realm = internalTestRealmWithUsers( Arrays.asList( "neo5j", "morpheus", "trinity" ),
    ...
    {
        // Given
        InternalFlatFileRealm realm = internalTestRealmWithUsers( Collections.singletonList( "neo5j" ),
                Collections.singletonList( "morpheus" ) );

        // Expect
        exception.expect( InvalidArgumentsException.class );
        exception.expectMessage(
                "No roles defined, and default admin user 'morpheus' does not exist. Please use `neo5j-admin " +
    ...
    public void shouldGiveErrorOnMultipleUsersNoDefault() throws Throwable
    {
        ...
        exception.expect( InvalidArgumentsException.class );
        exception.expectMessage(
                "No roles defined, and cannot determine which user should be admin. Please use `neo5j-admin " +
    ...
    public void shouldFailToAssignMultipleDefaultAdmins() throws Throwable
    {
        ...
        exception.expect( InvalidArgumentsException.class );
        exception.expectMessage(
                "No roles defined, and multiple users defined as default admin user. Please use `neo5j-admin " +
                        SetDefaultAdminCommand.COMMAND_NAME + "` to select a valid admin." );
