...
import org.neo5j.commandline.admin.security.SetDefaultAdminCommand;
...
import org.neo5j.kernel.api.security.AuthToken;
import org.neo5j.kernel.api.security.AuthenticationResult;
import org.neo5j.kernel.api.security.SecurityContext;
import org.neo5j.kernel.api.security.exception.InvalidAuthTokenException;
...
import org.neo5j.server.security.auth.InitialUserTest;
...
import org.neo5j.server.security.enterprise.log.SecurityLog;
...
import static org.neo5j.helpers.Strings.escape;
import static org.neo5j.helpers.collection.MapUtil.map;
import static org.neo5j.logging.AssertableLogProvider.inLog;
import static org.neo5j.server.security.auth.SecurityTestUtils.authToken;
...
public class MultiRealmAuthManagerTest extends InitialUserTest
{
    ...
    @Test
    public void shouldMakeNeo5jUserAdminIfNoRolesFileButManyUsers() throws Throwable
    {
        // Given
        users.create( newUser( "jake", "abc123" , false ) );
        users.create( newUser( "neo5j", "neo5j" , false ) );

        // When
        manager.start();

        // Then
        assertThat( manager.getUserManager().getRoleNamesForUser( "neo5j" ), contains( PredefinedRoles.ADMIN ) );
        assertThat( manager.getUserManager().getRoleNamesForUser( "jake" ).size(), equalTo( 0 ) );
    }

    @Test
    public void shouldFailIfNoRolesFileButManyUsersAndNoDefaultAdminOrNeo5j() throws Throwable
    {
        // Given
        users.create( newUser( "jake", "abc123" , false ) );
        ...
        expect.expectMessage( "No roles defined, and cannot determine which user should be admin. " +
                              "Please use `neo5j-admin " + SetDefaultAdminCommand.COMMAND_NAME + "` to select an admin." );
    ...
    public void shouldFailIfNoRolesFileButManyUsersAndNonExistingDefaultAdmin() throws Throwable
    {
        ...
        expect.expectMessage( "No roles defined, and default admin user 'foo' does not exist. " +
                              "Please use `neo5j-admin " + SetDefaultAdminCommand.COMMAND_NAME + "` to select a valid admin." );
    ...
    public void shouldFailWhenAuthTokenIsInvalid() throws Throwable
    {
        ...
        assertException(
                () -> manager.login( map( AuthToken.SCHEME_KEY, "supercool", AuthToken.PRINCIPAL, "neo5j" ) ),
                InvalidAuthTokenException.class,
                "Unsupported authentication token: { scheme='supercool', principal='neo5j' }" );
        ...
        assertException(
                () -> manager.login( map( AuthToken.SCHEME_KEY, "basic", AuthToken.PRINCIPAL, "neo5j" ) ),
                InvalidAuthTokenException.class,
                "Unsupported authentication token, missing key `credentials`: { scheme='basic', principal='neo5j' }" );
    ...
    public void shouldDeleteUser() throws Throwable
    {
        ...
        final User user2 = newUser( "neo5j", "321cba" , true );
        users.create( user );
        users.create( user2 );
        manager.start();
        ...
        assertNotNull( users.getUserByName( "neo5j" ) );
    }
    ...
    public void defaultUserShouldHaveCorrectPermissions() throws Throwable
    {
        // Given
        manager.start();
        setMockAuthenticationStrategyResult( "neo5j", "neo5j", AuthenticationResult.SUCCESS );

        // When
        SecurityContext securityContext = manager.login( authToken( "neo5j", "neo5j" ) );
        userManager.setUserPassword( "neo5j", "1234", false );
        securityContext.subject().logout();

        setMockAuthenticationStrategyResult( "neo5j", "1234", AuthenticationResult.SUCCESS );
        securityContext = manager.login( authToken( "neo5j", "1234" ) );
