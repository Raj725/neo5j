...
import org.neo5j.kernel.enterprise.api.security.EnterpriseSecurityContext;
...
import org.neo5j.server.security.enterprise.auth.EnterpriseAuthAndUserManager;
import org.neo5j.server.security.enterprise.auth.ProcedureInteractionTestBase;
...
import static org.neo5j.bolt.v1.messaging.util.MessageMatchers.msgRecord;
import static org.neo5j.bolt.v1.messaging.util.MessageMatchers.msgSuccess;
import static org.neo5j.bolt.v1.runtime.spi.StreamMatchers.eqRecord;
import static org.neo5j.bolt.v1.transport.integration.TransportTestUtil.eventuallyDisconnects;
import static org.neo5j.bolt.v1.transport.integration.TransportTestUtil.eventuallyReceives;
import static org.neo5j.server.security.enterprise.auth.LdapRealm.LDAP_AUTHORIZATION_FAILURE_CLIENT_MESSAGE;
import static org.neo5j.server.security.enterprise.auth.LdapRealm.LDAP_CONNECTION_TIMEOUT_CLIENT_MESSAGE;
import static org.neo5j.server.security.enterprise.auth.LdapRealm.LDAP_READ_TIMEOUT_CLIENT_MESSAGE;
...
@CreateLdapServer(
        ...
        keyStore = "target/test-classes/neo5j_ldap_test_keystore.jks",
...
public class LdapAuthIT extends EnterpriseAuthenticationTestBase
{
    ...
    private void restartNeo5jServerWithSaslDigestMd5() throws IOException
    {
    ...
    private void restartNeo5jServerWithSaslCramMd5() throws IOException
    {
        server.shutdownDatabase();
        server.ensureDatabase( asSettings( ldapOnlyAuthSettings.andThen(
                settings ->
    ...
    {
        restartNeo5jServerWithOverriddenSettings( ldapOnlyAuthSettings
    ...
    {
        assertAuthFail( "neo5j", "CANT_REMEMBER_MY_PASSWORDS_ANYMORE!" );
    }
    ...
    {
        // When
        restartNeo5jServerWithSaslDigestMd5();

        // Then
        assertAuth( "neo5j", MD5_HASHED_abc123 );
    }
    ...
    public void shouldFailToLoginWithLdapDigestMd5IfInvalidCredentials() throws Throwable
    {
        ...
        // Then
        assertAuthFail( "neo5j", MD5_HASHED_abc123.toUpperCase() );
    }
    ...
    {
        // When
        restartNeo5jServerWithSaslCramMd5();
    ...
    {
        restartNeo5jServerWithOverriddenSettings( settings -> settings.put( SecuritySettings.ldap_authorization_use_system_account, "false" ) );
    ...
    public void shouldBeAbleToLoginWithLdapAndAuthorizeInternally() throws Throwable
    {
        ...
        // Then
        // First login as the admin user 'neo5j' and create the internal user 'neo' with role 'reader'
    ...
    {
        // When
        restartNeo5jServerWithOverriddenSettings( ldapOnlyAuthSettings.andThen( settings ->
        {
    ...
    {
        restartNeo5jServerWithOverriddenSettings( activeDirectoryOnEc2NotUsingSystemAccountSettings );
    ...
    {
        restartNeo5jServerWithOverriddenSettings( activeDirectoryOnEc2UsingSystemAccountSettings );
    ...
    private void clearAuthCacheFromDifferentConnection() throws Exception
    {
        ...
        // Login as admin
        Map<String,Object> authToken = authToken( "neo5j", "abc123", null );
    ...
    private class EmbeddedTestCertificates implements AutoCloseable
    {
        ...
        {
            File keyStoreFile = fileFromResources( "/neo5j_ldap_test_keystore.jks" );
