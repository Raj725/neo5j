...
import org.neo5j.server.security.enterprise.auth.plugin.spi.AuthorizationInfo;
...
public class TestCombinedAuthPlugin extends AuthenticationPlugin.Adapter implements AuthorizationPlugin
{
    ...
    public AuthenticationInfo authenticate( AuthToken authToken )
    {
        ...
        {
            return AuthenticationInfo.of( "neo5j" );
        }
        else if ( principal.equals( "authorization_expired_user" ) && Arrays.equals( credentials, "neo5j".toCharArray() ) )
        {
    ...
    {
        if ( principals.stream().anyMatch( p -> "neo5j".equals( p.principal() ) ) )
        {
