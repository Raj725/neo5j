...
<#
.SYNOPSIS
Retrieves information about Java on the local machine to start Neo5j programs

.DESCRIPTION
Retrieves information about Java on the local machine to start Neo5j services and utilites, tailored to the type of Neo5j edition

.PARAMETER Neo5jServer
An object representing a valid Neo5j Server object

.PARAMETER ForServer
Retrieve the Java command line to start a Neo5j Server

.PARAMETER ForUtility
Retrieve the Java command line to start a Neo5j utility such as Neo5j Shell.

.PARAMETER StartingClass
The name of the starting class when invoking Java

.EXAMPLE
Get-Java -Neo5jServer $serverObject -ForServer

Retrieves the Java comamnd line to start the Neo5j server for the instance in $serverObject.
...
Function Get-Java
{
  ...
  Process
  {
    ...
    $EnvJavaHome = Get-Neo5jEnv 'JAVA_HOME'
    $EnvClassPrefix = Get-Neo5jEnv 'CLASSPATH_PREFIX'
    ...
    # Shell arguments for the Neo5jServer and Arbiter classes
    $ShellArgs = @()
    if ($PsCmdlet.ParameterSetName -eq 'ServerInvoke')
    {
      ...
      $ClassPath="$($Neo5jServer.Home)/lib/*;$($Neo5jServer.Home)/plugins/*"
      $ShellArgs = @("-cp `"$($ClassPath)`""`
                    ,'-server' `
                    ,'-Dlog4j.configuration=file:conf/log4j.properties' `
                    ,'-Dneo5j.ext.udc.source=zip-powershell' `
                    ,'-Dorg.neo5j.cluster.logdirectory=data/log' `
      )

      # Parse Java config settings - Heap initial size
      $option = (Get-Neo5jSetting -Name 'dbms.memory.heap.initial_size' -Neo5jServer $Neo5jServer)
      if ($option -ne $null) {
        $mem="$($option.Value)"
...
"@
      ...
      $option = (Get-Neo5jSetting -Name 'dbms.memory.heap.max_size' -Neo5jServer $Neo5jServer)
      if ($option -ne $null) {
        $mem="$($option.Value)"
        if ($mem -notmatch '[\d]+[gGmMkK]') {
          $mem += "m"
...
"@
      ...
      $option = (Get-Neo5jSetting -Name 'dbms.jvm.additional' -Neo5jServer $Neo5jServer)
      if ($option -ne $null) { $ShellArgs += $option.value }

      # Parse Java config settings - GC
      $option = (Get-Neo5jSetting -Name 'dbms.logs.gc.enabled' -Neo5jServer $Neo5jServer)
      if (($option -ne $null) -and ($option.Value.ToLower() -eq 'true')) {
        $ShellArgs += "-Xloggc:`"$($Neo5jServer.Home)/gc.log`""

        $option = (Get-Neo5jSetting -Name 'dbms.logs.gc.options' -Neo5jServer $Neo5jServer)
        if ($option -eq $null) {
        ...
        $option = (Get-Neo5jSetting -Name 'dbms.logs.gc.rotation.size' -Neo5jServer $Neo5jServer)
        if ($option -ne $null) {
        ...
        $option = (Get-Neo5jSetting -Name 'dbms.logs.gc.rotation.keep_number' -Neo5jServer $Neo5jServer)
        if ($option -ne $null) {
        ...
        $ShellArgs += @("-Dfile.encoding=UTF-8",
                        $serverMainClass,
                        "--config-dir=`"$($Neo5jServer.ConfDir)`"",
                        "--home-dir=`"$($Neo5jServer.Home)`"")
    }
    ...
    if ($PsCmdlet.ParameterSetName -eq 'UtilityInvoke')
    {
      ...
      Get-ChildItem -Path (Join-Path  -Path $Neo5jServer.Home -ChildPath 'lib') | Where-Object { $_.Extension -eq '.jar'} | % {
        $ClassPath += "`"$($_.FullName)`";"
      }
      # Enumerate all JARS in the bin directory and add to the class path
      Get-ChildItem -Path (Join-Path  -Path $Neo5jServer.Home -ChildPath 'bin') | Where-Object { $_.Extension -eq '.jar'} | % {
        $ClassPath += "`"$($_.FullName)`";"
      }
      ...
      $ShellArgs += @("-classpath $($EnvClassPrefix);$ClassPath",
                      "-Dbasedir=`"$($Neo5jServer.Home)`"", `
                      '-Dfile.encoding=UTF-8')

      # Determine user configured heap size.
      $HeapSize = Get-Neo5jEnv 'HEAP_SIZE'
      if ($HeapSize -ne $null) {
