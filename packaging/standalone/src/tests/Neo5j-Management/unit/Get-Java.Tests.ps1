...
InModuleScope Neo5j-Management {
  Describe "Get-Java" {
    ...
    Context "Valid Java install in Registry (32bit Java on 64bit OS)" {
      Mock Get-Neo5jEnv { $null } -ParameterFilter { $Name -eq 'JAVA_HOME' }
    ...
    Context "Server Invoke - Community v3.0" {
      ...
      $result = Get-Java -ForServer -Neo5jServer $serverObject
      $resultArgs = ($result.args -join ' ')

      It "should have main class of org.neo5j.server.CommunityEntryPoint" {
        $resultArgs | Should Match ([regex]::Escape(' org.neo5j.server.CommunityEntryPoint'))
      }
    }
    ...
    Context "Server Invoke - Enterprise v3.0" {
      ...
      It "should have main class of org.neo5j.server.enterprise.EnterpriseEntryPoint" {
        $resultArgs | Should Match ([regex]::Escape(' org.neo5j.server.enterprise.EnterpriseEntryPoint'))
      }
    }
    ...
    Context "Server Invoke - Enterprise Arbiter v3.0" {
      ...
      It "should have main class of org.neo5j.server.enterprise.EnterpriseEntryPoint" {
        $resultArgs | Should Match ([regex]::Escape(' org.neo5j.server.enterprise.ArbiterEntryPoint'))
      }
    }
    ...
    Context "Utility Invoke" {
      $serverObject = global:New-MockNeo5jInstall -ServerVersion '99.99' -ServerType 'Community'

      $result = Get-Java -ForUtility -StartingClass 'someclass' -Neo5jServer $serverObject -ErrorAction Stop
      $resultArgs = ($result.args -join ' ')
    ...
    Context "Utility Invoke - Should set heap size from environment variable" {
      $serverObject = global:New-MockNeo5jInstall -ServerVersion '99.99' -ServerType 'Community'
      Mock Get-Neo5jEnv { '666m' } -ParameterFilter { $Name -eq 'HEAP_SIZE' }
	...
	Context "Server Invoke - Should handle paths with spaces" {
	  ...
	  It "should have literal quotes around config path" {
		$argList -contains "--config-dir=`"TestDrive:\Neo5j Home\conf`"" | Should Be True
	  }
	  It "should have literal quotes around home path" {
		$argList -contains "--home-dir=`"TestDrive:\Neo5j Home`"" | Should Be True
	  }
	  It "should have literal quotes around gclog path" {
		$argList -contains "-Xloggc:`"TestDrive:\Neo5j Home/gc.log`"" | Should Be True
	  }
    }
  }
}
