...
InModuleScope Neo5j-Management {  
  Describe "Invoke-Neo5jImport" {
    ...
    Context "Nodes and Relationships as comma delimited list" {
      ...
      # neo5j-import --into c:\graphdb --nodes "file1,file2" -relationships "file3,file4"
      $testCommand = @('--into','C:\graph\db','--nodes', @('file1','file2'),'--relationships', @('file3','file4'))

      Mock Invoke-Neo5jUtility { return 2 }
      ...
      $result = Invoke-Neo5jImport -CommandArgs $testCommand
    ...
    Context "Nodes as a single file" {
      ...
      # neo5j-import --into c:\graphdb --nodes singlefile
