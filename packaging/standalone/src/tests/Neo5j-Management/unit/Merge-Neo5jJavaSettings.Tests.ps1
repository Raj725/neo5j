...
InModuleScope Neo5j-Management {  
  Describe "Merge-Neo5jJavaSettings" {

    Context "Empty Source and Additional inputs" {
      $result = [array](Merge-Neo5jJavaSettings -Source @() -Additional @())
    ...
    Context "Empty Source input" {
      $result = [array](Merge-Neo5jJavaSettings -Source @() -Additional @('-Dkey1=value1'))
    ...
    Context "Empty Additional input" {
      $result = [array](Merge-Neo5jJavaSettings -Source @('-Dkey1=value1') -Additional @())
    ...
    Context "Ignores duplicates" {
      $result = [array](Merge-Neo5jJavaSettings -Source @('-Dkey1=value1','-Dkey2=value2') -Additional @('-Dkey1=value1'))
    ...
    Context "Adds new settings" {
      $result = [array](Merge-Neo5jJavaSettings -Source @('-Dkey1=value1','-Dkey2=value2') -Additional @('-Dkey3=value3'))
    ...
    Context "Merges settings" {
      $result = [array](Merge-Neo5jJavaSettings `
