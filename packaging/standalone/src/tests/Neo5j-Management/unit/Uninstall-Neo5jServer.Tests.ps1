...
InModuleScope Neo5j-Management {
  Describe "Uninstall-Neo5jServer" {
    ...
    Context "Missing service name in configuration files" {
      $serverObject = global:New-MockNeo5jInstall -WindowsService $null

      It "throws error for missing service name in configuration file" {
        { Uninstall-Neo5jServer -Neo5jServer $serverObject -ErrorAction Stop } | Should Throw
      }
    }
    ...
    Context "Windows service does not exist" {
      ...
      $result = Uninstall-Neo5jServer -Neo5jServer $serverObject
