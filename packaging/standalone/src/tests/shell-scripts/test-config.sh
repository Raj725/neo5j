...
test_expect_success "http: should read port and address from config" "
  clear_config &&
  set_config 'dbms.connector.http.address' '1.2.3.4:1234' neo5j.conf &&
  set_config 'dbms.connector.http.enabled' 'true' neo5j.conf &&
...
test_expect_success "http: should read only port from config" "
  clear_config &&
  set_config 'dbms.connector.http.address' ':1234' neo5j.conf &&
  set_config 'dbms.connector.http.enabled' 'true' neo5j.conf &&
...
test_expect_success "http: should fallback to default listening address if defined" "
  clear_config &&
  set_config 'dbms.connectors.default_listen_address' '100.200.300.400' neo5j.conf &&
  set_config 'dbms.connector.http.enabled' 'true' neo5j.conf &&
...
test_expect_success "http: should read port and address from config and prioritize non-deprecated" "
  clear_config &&
  set_config 'dbms.connector.http.address' '1.2.3.4:1234' neo5j.conf &&
  set_config 'dbms.connector.http.listen_address' 'a.b.c.d:333' neo5j.conf &&
  set_config 'dbms.connector.http.enabled' 'true' neo5j.conf &&
...
test_expect_success "https: should display default https if http disabled and no other options" "
  clear_config &&
  set_config 'dbms.connector.http.enabled' 'false' neo5j.conf &&
...
test_expect_success "https: should read port and address from config" "
  clear_config &&
  set_config 'dbms.connector.http.enabled' 'false' neo5j.conf &&
  set_config 'dbms.connector.https.address' '1.2.3.4:1234' neo5j.conf &&
  set_config 'dbms.connector.https.enabled' 'true' neo5j.conf &&
...
test_expect_success "https: should read only port from config" "
  clear_config &&
  set_config 'dbms.connector.http.enabled' 'false' neo5j.conf &&
  set_config 'dbms.connector.https.address' ':1234' neo5j.conf &&
  set_config 'dbms.connector.https.enabled' 'true' neo5j.conf &&
...
test_expect_success "https: should read port and address from config and prioritize non-deprecated" "
  ...
  set_config 'dbms.connector.http.enabled' 'false' neo5j.conf &&
  set_config 'dbms.connector.https.address' '1.2.3.4:1234' neo5j.conf &&
  set_config 'dbms.connector.https.listen_address' 'a.b.c.d:333' neo5j.conf &&
  set_config 'dbms.connector.https.enabled' 'true' neo5j.conf &&
...
test_expect_success "should write a specific message in HA mode" "
  clear_config &&
  set_config 'dbms.mode' 'HA' neo5j.conf &&
...
test_expect_success "should respect log directory configuration" "
  clear_config &&
  mkdir -p '$(neo5j_home)/other-log-dir' &&
  set_config 'dbms.directories.logs' 'other-log-dir' neo5j.conf &&
  run_daemon &&
  test_expect_file_matching 'stdout from java' '$(neo5j_home)/other-log-dir/neo5j.log'
"

test_expect_success "can configure log directory outside neo5j-root" "
  clear_config &&
  mkdir -p other-log-dir &&
  set_config 'dbms.directories.logs' '$(pwd)/other-log-dir' neo5j.conf &&
  run_daemon &&
  test_expect_file_matching 'stdout from java' '$(pwd)/other-log-dir/neo5j.log'
"

test_expect_success "should write paths in use" "
  clear_config &&
  mkdir '$(neo5j_home)/loogs' &&
  mkdir '$(neo5j_home)/ruun' &&
  set_config 'dbms.directories.data' '/data/bob' neo5j.conf &&
  set_config 'dbms.directories.import' '/import/bob' neo5j.conf &&
  set_config 'dbms.directories.plugins' '/plugins/bob' neo5j.conf &&
  set_config 'dbms.directories.certificates' '/certs/bob' neo5j.conf &&
  set_config 'dbms.directories.run' 'ruun' neo5j.conf &&
  set_config 'dbms.directories.logs' 'loogs' neo5j.conf &&
  test_expect_stdout_matching '  home:         $(neo5j_home)' run_daemon &&
  test_expect_stdout_matching '  config:       $(neo5j_home)/conf' run_daemon &&
  ...
  test_expect_stdout_matching '  logs:         $(neo5j_home)/loogs' run_daemon &&
  test_expect_stdout_matching '  run:          $(neo5j_home)/ruun' run_daemon
"

test_expect_success "should write active database" "
  clear_config &&
  set_config 'dbms.active_database' 'bigdata' neo5j.conf &&
