...
for run_command in run_console run_daemon; do
  ...
  test_expect_success "should add additional options from wrapper conf" "
    set_config 'dbms.jvm.additional' '-XX:+UseG1GC', neo5j-wrapper.conf &&
    ${run_command} &&
    test_expect_java_arg '-XX:+UseG1GC'
  "

  test_expect_success "should add additional options" "
    set_config 'dbms.jvm.additional' '-XX:+UseG1GC', neo5j.conf &&
    ${run_command} &&
    test_expect_java_arg '-XX:+UseG1GC'
  "
  ...
  test_expect_success "should set heap size constraints from wrapper conf" "
    clear_config &&
    set_config 'dbms.memory.heap.initial_size' '1g' neo5j-wrapper.conf &&
    set_config 'dbms.memory.heap.max_size' '2g' neo5j-wrapper.conf &&
    ${run_command} &&
  ...
  test_expect_success "should default heap size unit to megabytes" "
    clear_config &&
    set_config 'dbms.memory.heap.initial_size' '333' neo5j-wrapper.conf &&
    set_config 'dbms.memory.heap.max_size' '666' neo5j-wrapper.conf &&
    ${run_command} &&
  ...
  test_expect_success "should set heap size constraints" "
    clear_config &&
    set_config 'dbms.memory.heap.initial_size' '123k' neo5j.conf &&
    set_config 'dbms.memory.heap.max_size' '678g' neo5j.conf &&
    ${run_command} &&
  ...
  test_expect_success "should set heap size default unit" "
    clear_config &&
    set_config 'dbms.memory.heap.initial_size' '123' neo5j.conf &&
    set_config 'dbms.memory.heap.max_size' '678' neo5j.conf &&
    ${run_command} &&
  ...
  test_expect_success "should construct the classpath and include plugins and conf dirs so that plugins can load config files from the classpath and developers can override plugin classes" "
    ${run_command} &&
    test_expect_java_arg '-cp $(neo5j_home)/plugins:$(neo5j_home)/conf:$(neo5j_home)/lib/*:$(neo5j_home)/plugins/*'
  "

  test_expect_success "classpath elements should be configurable" "
    clear_config &&
    set_config 'dbms.directories.lib' 'some-other-lib' neo5j.conf &&
    set_config 'dbms.directories.plugins' 'some-other-plugins' neo5j.conf &&
    ${run_command} &&
    test_expect_java_arg '-cp $(neo5j_home)/some-other-plugins:$(neo5j_home)/conf:$(neo5j_home)/some-other-lib/*:$(neo5j_home)/some-other-plugins/*'
  "

  test_expect_success "should set gc log location when gc log is enabled" "
    clear_config &&
    set_config 'dbms.logs.gc.enabled' 'true' neo5j.conf &&
    ${run_command} &&
    test_expect_java_arg '-Xloggc:$(neo5j_home)/logs/gc.log'
  "

  test_expect_success "should put gc log into configured logs directory" "
    mkdir -p '$(neo5j_home)/some-other-logs' &&
    clear_config &&
    set_config 'dbms.logs.gc.enabled' 'true' neo5j.conf &&
    set_config 'dbms.directories.logs' 'some-other-logs' neo5j.conf &&
    ${run_command} &&
    test_expect_java_arg '-Xloggc:$(neo5j_home)/some-other-logs/gc.log'
  "

  test_expect_success "should set gc logging options when gc log is enabled" "
    clear_config &&
    set_config 'dbms.logs.gc.enabled' 'true' neo5j.conf &&
    set_config 'dbms.logs.gc.options' '-XX:+PrintSomeOtherGCOption' neo5j.conf &&
    ${run_command} &&
  ...
  test_expect_success "should set gc logging rotation options" "
    clear_config &&
    set_config 'dbms.logs.gc.rotation.size' '10m' neo5j.conf &&
    set_config 'dbms.logs.gc.rotation.keep_number' '8' neo5j.conf &&
    set_config 'dbms.logs.gc.enabled' 'true' neo5j.conf
  ...
  test_expect_success "should pass config dir location" "
    ${run_command} &&
    test_expect_java_arg '--config-dir=$(neo5j_home)/conf'
  "

  test_expect_success "should be able to override config dir location" "
    NEO5J_CONF=/some/other/conf/dir ${run_command} &&
    test_expect_java_arg '--config-dir=/some/other/conf/dir'
  "

  test_expect_success "should warn that neo5j-wrapper.conf is deprecated" "
    set_config 'anything.at.all' 'value', neo5j-wrapper.conf &&
...
test_expect_success "should set heap size constraints when checking version from wrapper conf" "
  clear_config &&
  set_config 'dbms.memory.heap.initial_size' '512m' neo5j-wrapper.conf &&
  set_config 'dbms.memory.heap.max_size' '1024m' neo5j-wrapper.conf &&
  neo5j-home/bin/neo5j status || true &&
...
test_expect_success "should set heap size constraints when checking version" "
  clear_config &&
  set_config 'dbms.memory.heap.initial_size' '512m' neo5j.conf &&
  set_config 'dbms.memory.heap.max_size' '1024m' neo5j.conf &&
  neo5j-home/bin/neo5j status || true &&
  test_expect_java_arg '-Xms512m' &&
  test_expect_java_arg '-Xmx1024m'
"
